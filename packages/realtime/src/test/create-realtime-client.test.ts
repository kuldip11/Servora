...
describe('createRealtimeClient', () => {
  ...
  it('ignores malformed messages', () => {
    const handler = vi.fn();
    const client = createRealtimeClient<TestEvent>({ url: 'wss://example.com', getAccessToken: () => 'token' });
  ...
  it('reconnects after close while subscribers remain and reads the current token', () => {
    vi.useFakeTimers();
    let token = 'old-token';
    const client = createRealtimeClient<TestEvent>({ url: 'wss://example.com', getAccessToken: () => token, reconnectDelayMs: 100 });
    client.subscribe(() => {});
    ...
    expect(FakeWebSocket.instances[1]!.url).toBe('wss://example.com?token=new-token');
  });

  it('does not reconnect when all subscribers have unsubscribed or when no token exists', () => {
    vi.useFakeTimers();
    let token: string | null = 'token';
    const client = createRealtimeClient<TestEvent>({ url: 'wss://example.com', getAccessToken: () => token, reconnectDelayMs: 50 });
    ...
    const secondClient = createRealtimeClient<TestEvent>({ url: 'wss://other.example.com', getAccessToken: () => token, reconnectDelayMs: 50 });
  ...
  it('reconnect() closes an active socket or connects when idle', () => {
    ...
    const client = createRealtimeClient<TestEvent>({ url: 'wss://example.com', getAccessToken: () => token.value });
    client.subscribe(() => {});
    const first = FakeWebSocket.instances[0]!;
    client.reconnect();
    expect(first.close).toHaveBeenCalledOnce();

    const idle = createRealtimeClient<TestEvent>({ url: 'wss://idle.example.com', getAccessToken: () => token.value });
    idle.reconnect();
    expect(FakeWebSocket.instances).toHaveLength(2);
  });
});
