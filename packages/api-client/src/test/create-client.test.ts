...
type Interceptor<T> = { use: (fulfilled: (value: T) => any, rejected?: (error: any) => any) => void };
...
function storage(overrides: Partial<TokenStorageAdapter> = {}): TokenStorageAdapter {
  return {
    getAccessToken: () => null,
...
describe('createApiClient request interceptor', () => {
  it('creates the client with JSON headers and injects access/context headers', async () => {
    ...
    const s = storage({ getAccessToken: () => 'access', getTenantId: () => 'tenant', getBranchId: () => 'branch' });
    createApiClient({ baseURL: 'https://api.example.com', timeout: 5000, storage: s, onRefreshFailure: vi.fn() });
  ...
  it('removes stale tenant and branch headers when no context is selected', () => {
    ...
    createApiClient({ baseURL: '/api', timeout: 1000, storage: s, onRefreshFailure: vi.fn() });
    const config = { headers: { 'X-Tenant-ID': 'old-tenant', 'X-Branch-ID': 'old-branch', Authorization: 'old' } as Record<string, string> };
    const result = installedInterceptors(harness).requestHandler(config);
...
describe('createApiClient response interceptor', () => {
  it('rejects non-401 errors without attempting refresh', async () => {
    const harness = makeClientHarness();
    const failure = vi.fn();
    createApiClient({ baseURL: '/api', timeout: 1000, storage: storage(), onRefreshFailure: failure });
    const error = { response: { status: 500 }, config: { url: '/orders' } };
    await expect(installedInterceptors(harness).responseRejected(error)).rejects.toBe(error);
  ...
  it('rejects a 401 for the refresh endpoint and an already retried request', async () => {
    ...
    await expect(handler({ response: { status: 401 }, config: { url: '/api/auth/refresh' } })).rejects.toBeTruthy();
    await expect(handler({ response: { status: 401 }, config: { url: '/orders', _retry: true } })).rejects.toBeTruthy();
    expect(mocks.post).not.toHaveBeenCalled();
  });
  ...
  it('refreshes tokens, updates storage, retries the request, and returns the retry result', async () => {
    ...
    mocks.post.mockResolvedValue({ data: { data: { accessToken: 'new-access', refreshToken: 'new-refresh' } } });
    createApiClient({ baseURL: '/api', timeout: 1000, storage: s, onRefreshFailure: failure });

    const original = { url: '/orders', headers: {} as Record<string, string> };
    const result = await installedInterceptors(harness).responseRejected({ response: { status: 401 }, config: original });
    expect(mocks.post).toHaveBeenCalledWith('/api/auth/refresh', { refreshToken: 'refresh' });
  ...
  it('queues concurrent 401 requests behind one refresh call', async () => {
    ...
    const refresh = Promise.resolve({ data: { data: { accessToken: 'new-access', refreshToken: 'new-refresh' } } });
  ...
  it('rejects the refresh error and fails queued requests when refresh fails', async () => {
    ...
    createApiClient({ baseURL: '/api', timeout: 1000, storage: s, onRefreshFailure: failure });
    const handler = installedInterceptors(harness).responseRejected;
    const first = handler({ response: { status: 401 }, config: { url: '/one', headers: {} } });
    const second = handler({ response: { status: 401 }, config: { url: '/two', headers: {} } });
