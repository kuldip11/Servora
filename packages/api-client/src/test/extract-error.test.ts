...
describe('extractApiError', () => {
  it('returns the API message from an Axios error', () => {
    const error = new axios.AxiosError('network message', 'ERR_BAD_REQUEST', undefined, undefined, {
      status: 400,
  ...
  it('returns a regular Error message', () => {
    expect(extractApiError(new Error('Something failed'))).toBe('Something failed');
  });

  it('returns a stable fallback for unknown values', () => {
    expect(extractApiError({ reason: 'unknown' })).toBe('An unexpected error occurred');
  });
});
