...
describe("createPaymentSchema", () => {
  it("accepts each supported payment method at the minimum amount", () => {
    for (const method of [
      ...
      expect(
        createPaymentSchema.safeParse({ orderId: uuid, method, amount: 0.01 })
          .success,
      ).toBe(true);
    }
  });
...
describe("createRefundSchema", () => {
  ...
  it("requires positive amount and a 1-500 character reason", () => {
    expect(
      createRefundSchema.safeParse({ paymentId: uuid, amount: 0, reason: "x" })
        .success,
    ).toBe(false);
    expect(
      createRefundSchema.safeParse({ paymentId: uuid, amount: 1, reason: "" })
        .success,
    ).toBe(false);
    expect(
      createRefundSchema.safeParse({
