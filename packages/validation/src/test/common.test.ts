...
describe("paginationSchema", () => {
  ...
  it("enforces page and limit boundaries", () => {
    ...
    expect(paginationSchema.safeParse({ page: 1, limit: 100 }).success).toBe(
      true,
    );
  });
});
...
describe("branchFormSchema", () => {
  ...
  it("enforces name, address, and phone lengths", () => {
    expect(
      branchFormSchema.safeParse({ ...branch, name: " ".repeat(151) }).success,
    ).toBe(false);
    expect(
      branchFormSchema.safeParse({ ...branch, address: "a".repeat(501) })
        .success,
    ).toBe(false);
    expect(
      branchFormSchema.safeParse({ ...branch, phone: "1".repeat(31) }).success,
    ).toBe(false);
  });
});
