...
import { describedBy, FieldFooter, FieldLabel, fieldBaseClasses, useFieldIds } from '../shared';
...
describe('form shared helpers', () => {
  it('generates stable field ids and chooses error over hint', () => {
    ...
    expect(result.current).toEqual({ fieldId: 'email', hintId: 'email-hint', errorId: 'email-error' });
  ...
  it('renders labels, errors, hints and character counts', () => {
    render(<><FieldLabel htmlFor="name" required>Name</FieldLabel><input id="name" aria-label="Name field" /><FieldFooter hint="Help" hintId="h" errorId="e" charCount={12} maxLength={10} /></>);
