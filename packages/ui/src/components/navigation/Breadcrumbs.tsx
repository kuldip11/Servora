        ...
        {items.map((item, i) => {
          ...
          return (
            <li key={item.label} className="flex items-center gap-1.5">
              {i > 0 && <ChevronRight className="w-3.5 h-3.5 text-text-disabled shrink-0" aria-hidden="true" />}
              {isLast ? (
                <span aria-current="page" className="text-text-primary font-medium">
                  {item.label}
                </span>
              ) : As ? (
                <As {...item.linkProps} className="text-text-secondary hover:text-text-primary transition-colors duration-fast ease-standard">
                  {item.label}
                </As>
              ) : (
                <a
