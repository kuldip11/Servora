...
export function Sidebar({
  ...
  return (
    <TooltipProvider delayDuration={300}>
      ...
      >
        {header && (
          <div className={cn('shrink-0 px-4 py-4 border-b border-border', collapsed && 'px-2')}>
            {header}
          </div>
        )}
              ...
              {section.items.map((item) => {
                const row = (
                  ...
                  >
                    ...
                    {!collapsed && <span className="flex-1 min-w-0 truncate text-left">{item.label}</span>}
                    {!collapsed && item.badge && <span className="shrink-0">{item.badge}</span>}
                  </NavLink>
                );
                return collapsed ? (
                  <Tooltip key={item.label} trigger={row} content={item.label} side="right" />
                ) : (
                  row
                );
              })}
        ...
        {footer && (
          <div className={cn('shrink-0 px-3 py-3 border-t border-border', collapsed && 'px-2')}>
            {footer}
          </div>
        )}
