...
import { buildRows, filterOptions, rowDomId, useActiveRow, useDebouncedValue, useTypeaheadBuffer } from '../shared';

const options = [{ value: '1', label: 'Apple' }, { value: '2', label: 'Banana', group: 'Fruit' }, { value: '3', label: 'Disabled', disabled: true }];
...
describe('selection shared', () => {
  ...
  it('tracks active rows and typeahead prefixes', () => {
    const commit = vi.fn(); const { result } = renderHook(() => useActiveRow(buildRows(options), commit));
    ...
    const onPrefix = vi.fn(); renderHook(() => useTypeaheadBuffer(onPrefix));
  });
  it('debounces a changing value', async () => {
    vi.useFakeTimers();
    const { result, rerender } = renderHook(({ value }) => useDebouncedValue(value, 50), { initialProps: { value: 'a' } });
    rerender({ value: 'ab' });
