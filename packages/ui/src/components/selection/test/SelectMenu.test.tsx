...
describe('SelectMenu', () => {
  ...
  it('opens and commits an option', async () => {
    ...
    render(<SelectMenu label="Choice" options={options} value={undefined} onChange={onChange} />);
    await user.click(screen.getByRole('combobox', { name: 'Choice' }));
    await user.click(screen.getByRole('option', { name: 'Two' }));
    expect(onChange).toHaveBeenCalledWith('two');
  });
  it('shows errors accessibly', () => {
    render(<SelectMenu label="Choice" options={options} value={undefined} onChange={() => {}} error="Required" />);
