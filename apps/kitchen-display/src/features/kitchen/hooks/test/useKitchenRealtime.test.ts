...
vi.mock("../../../../shared/lib/realtime", () => ({
  ...
  useRealtimeEvent: (type: string, handler: (event: any) => void) => mocks.handlers.push([type, handler]),
}));
...
describe("useKitchenRealtime", () => {
  it("upserts full ticket payloads directly into the KDS cache", () => {
    ...
    expect(mocks.handlers.map(([type]) => type)).toEqual(["kitchen.ticket.created", "kitchen.ticket.updated"]);
    const ticket = { id: "t1", status: "FIRED", firedAt: "2026-08-27T10:00:00.000Z" };
    mocks.handlers[0]![1]({ type: "kitchen.ticket.created", payload: ticket });
    expect(mocks.setQueryData).toHaveBeenCalledWith(KITCHEN_TICKETS_QUERY_KEY, expect.any(Function));
  ...
  it("removes served tickets from the visible queue", () => {
    useKitchenRealtime();
    const updaterCall = () => {
      mocks.handlers[1]![1]({ type: "kitchen.ticket.updated", payload: { id: "t1", status: "SERVED", firedAt: "2026-08-27T10:00:00.000Z" } });
