...
vi.mock('../../api/tickets', () => ({ updateTicketStatus: mocks.updateTicketStatus }));
...
describe('useUpdateTicketStatus', () => {
  it('configures mutation callbacks', async () => {
    ...
    expect(mocks.toast).toHaveBeenCalledWith({ title: 'Ticket updated', tone: 'success' });
    options.onError();
    expect(mocks.toast).toHaveBeenCalledWith({ title: 'Failed to update ticket', tone: 'danger' });
  });
});
