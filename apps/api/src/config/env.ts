...
const schema = z.object({
  ...
  CORS_ORIGIN: z.string().default("http://localhost:5173,http://localhost:5176"),
...
export function loadApiEnv(source: NodeJS.ProcessEnv = process.env): ApiEnv {
  ...
  if (value.NODE_ENV === "production") {
    if (value.JWT_SECRET.length < 32 || value.REFRESH_TOKEN_SECRET.length < 32) {
      throw new Error("Invalid API environment: production JWT secrets must be at least 32 characters");
    }
    const origins = value.CORS_ORIGIN.split(",").map((origin) => origin.trim()).filter(Boolean);
    if (origins.includes("*")) {
      throw new Error("Invalid API environment: wildcard CORS is forbidden in production");
    }
    for (const origin of origins) {
      const url = new URL(origin);
      if (url.protocol !== "https:" || ["localhost", "127.0.0.1"].includes(url.hostname)) {
        throw new Error(`Invalid API environment: production CORS origin must be a public HTTPS URL (${origin})`);
      }
    }
  }
  return value;
