...
describe("membership context", () => {
  it("lists active memberships and global-owner status", async () => {
    const db = {
      query: {
        tenantMemberships: {
          findMany: vi.fn().mockResolvedValue([activeMembership]),
        },
        users: {
  ...
  it("rejects a membership that does not match the active context", async () => {
    const db = {
      query: {
        tenantMemberships: { findFirst: vi.fn().mockResolvedValue(undefined) },
    ...
    ).rejects.toThrow("Membership access denied");
  });
  ...
  it("allows an assigned branch and preserves context", async () => {
    const db = {
      query: {
        tenantMemberships: {
          findFirst: vi.fn().mockResolvedValue(activeMembership),
        },
        branches: {
          findFirst: vi.fn().mockResolvedValue({ id: "b1", tenantId: "t1" }),
        },
  ...
  it("rejects a branch not assigned to a branch-scoped membership", async () => {
    const membership = {
      ...
      branches: [{ branchId: "other", branch: {} }],
    };
    const db = {
      query: {
