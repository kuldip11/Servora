...
import { db } from '../../db';
import { resolveAuthorization, resolveMembership } from '../../lib/authorization/authorization';
import { globalUserRoles } from '../../db/schema';
import { eq } from 'drizzle-orm';
import { resolveActiveBranch } from '../../lib/authorization/membership-context';
import { UnauthorizedError, ForbiddenError, MissingBranchError } from '../errors';
...
export const requireAuthPlugin = () =>
  new Elysia({ name: 'require-auth' })
    ...
    .derive({ as: 'global' }, async ({ headers, requestContext }): Promise<{ auth: AuthContext; logger: Logger }> => {
      ...
      const legacyPayload = payload as JwtPayload & { tenantId?: string; membershipId?: string; branchId?: string | null };
      if (legacyPayload.tenantId || legacyPayload.membershipId || legacyPayload.branchId) {
      ...
      if (!requestedTenant) {
        ...
        const logger = createLogger({ requestId: requestContext.requestId, userId: auth.userId }, 'app');
        return { auth, logger };
      }

      const membership = await resolveMembership(db, payload.sub, requestedTenant);
      if (!membership) {
      ...
      if (requestedBranch && requestedBranch !== 'all') {
        const active = await resolveActiveBranch(db, baseContext, requestedBranch);
      ...
      const decision = await resolveAuthorization(db, { ...baseContext, branchId });
      if (!decision.allowed) {
        throw new ForbiddenError('Franchise access denied');
      }
      ...
      const roles = [
        ...new Set([
          ...
          ...membership.roles.map((item: any) => item.role?.name ?? item.roleId),
        ]),
      ] as RoleName[];
...
export function requireRoles(auth: AuthContext, allowed: RoleName[]): void {
  ...
  if (!hasRole) throw new ForbiddenError('Insufficient permissions', { required: allowed, actual: auth.roles });
}
...
export function requireBranch(auth: AuthContext, message = 'Please select a specific branch.'): string {
