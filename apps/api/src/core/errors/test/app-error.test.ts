...
import {
  AppError, ConflictError, DomainRuleError, ErrorCode, ForbiddenError,
  InternalError, MissingBranchError, NotFoundError, ServiceUnavailableError,
...
describe('AppError hierarchy', () => {
  it('preserves code, status, details and serializable response shape', () => {
    ...
    expect(error.toJSON()).toMatchObject({ success: false, code: ErrorCode.VALIDATION_FAILED, message: 'Bad input', details: { field: 'name' } });
  ...
  it('formats not-found messages with and without ids', () => {
    ...
    expect(new NotFoundError('Tenant', 'abc').message).toBe('Tenant with id abc not found');
  });
