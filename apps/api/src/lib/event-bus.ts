...
export const eventBus = {
  async publish(event: RealtimeEvent, tenantId: string, branchId?: string): Promise<void> {
    const message = JSON.stringify({ ...event, tenantId, branchId, timestamp: new Date().toISOString() });
