...
describe('jwt helpers', () => {
  const user = { id: 'u1', email: 'u@example.com', roles: [{ name: 'OWNER', permissions: [{ key: 'tenant:read' }, { key: 'tenant:read' }] }] } as any;
  it('signs and verifies access tokens with identity and deduplicated permissions', () => {
    ...
    expect(payload).toMatchObject({ sub: 'u1', email: 'u@example.com', roles: ['OWNER'], permissions: ['tenant:read'] });
