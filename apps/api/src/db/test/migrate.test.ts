...
async function loadMigrate(options: {
  ...
  const migrationClient = Object.assign(
    vi.fn(async (strings: TemplateStringsArray) => {
      ...
      if (sql.includes('information_schema.tables') && sql.includes("table_name = 'tenants'")) {
  ...
  vi.doMock('fs', () => ({ existsSync: vi.fn(() => options.hasMigrationsFolder) }));
...
describe('db migrate entrypoint', () => {
  it('exits cleanly when no migrations folder exists', async () => {
    ...
    const { migrationClient, migrate } = await loadMigrate({ hasMigrationsFolder: false });
  ...
  it('runs drizzle migrations after confirming schema history is consistent', async () => {
    ...
    expect(migrate).toHaveBeenCalledWith({}, { migrationsFolder: './src/db/migrations' });
    expect(migrationClient.end).toHaveBeenCalled();
  });
});
