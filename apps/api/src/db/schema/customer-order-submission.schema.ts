...
export const customerOrderSubmissions = pgTable(
  ...
  (t) => ({
    sessionKeyUnique: uniqueIndex("customer_order_submissions_session_key_unique").on(t.customerSessionId, t.idempotencyKey),
    sessionIdx: index("customer_order_submissions_session_idx").on(t.customerSessionId),
  }),
);
