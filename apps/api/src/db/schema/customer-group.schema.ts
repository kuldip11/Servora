...
export const customerGroups = pgTable(
  ...
  (t) => ({
    tenantNameUnique: uniqueIndex("customer_groups_tenant_name_unique").on(
      t.tenantId,
      t.name,
    ),
    tenantIdx: index("customer_groups_tenant_idx").on(t.tenantId),
    discountAtMostOne: check(
      "customer_groups_discount_at_most_one",
      sql`NOT (${t.discountPercent} IS NOT NULL AND ${t.discountFixed} IS NOT NULL)`,
    ),
