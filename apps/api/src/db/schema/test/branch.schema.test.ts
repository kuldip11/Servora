...
describe('branch.schema.ts', () => {
  it('defines branches with its contract columns', () => {
    expectTable(branches, 'branches', ['id', 'tenantId', 'name', 'address', 'phone', 'isActive', 'dineInEnabled', 'takeawayEnabled', 'deliveryEnabled', 'onlineEnabled', 'tablesEnabled', 'createdAt', 'updatedAt']);
  });
});
