...
async function runMigrations() {
  ...
  if (!existsSync(migrationsFolder)) {
    ...
    console.log('   Run `bun run db:generate` first to generate SQL migrations from the schema,');
    console.log('   OR run `bun run db:push` to push the schema directly (dev only).');
  ...
  if (hasMigrationHistory) {
    const countRows = await migrationClient`SELECT count(*)::int AS count FROM "drizzle"."__drizzle_migrations"`;
    historyRowCount = Number(countRows[0]?.['count'] ?? 0);
  }

  if (hasAppSchema && historyRowCount === 0) {
    console.error('❌ Database schema already exists, but no migration history is recorded.');
    console.error('');
    console.error('   This usually means `drizzle-kit push` was run against this database instead of');
    console.error('   `db:migrate` — push syncs the schema directly and skips the migrations folder');
    console.error('   entirely, so RBAC reference-data migrations (roles/permissions) never ran, and');
    console.error('   drizzle has no record of what\'s already applied.');
    console.error('');
    console.error('   Fix: reset and re-migrate through the migrations folder so history stays');
    ...
    console.error('');
    console.error('   Avoid `db:push` on this project — required RBAC reference data lives in raw SQL');
    console.error('   migrations, not in schema.ts, so push can never install it.');
    await migrationClient.end();
    process.exit(1);
  }
...
runMigrations().catch((err: any) => {
  ...
  if (process.env['NODE_ENV'] !== 'production' && (err?.code === '42710' || err?.code === '42P07')) {
    console.error('');
    console.error('💡 Development database appears to contain objects that are not aligned with Drizzle migration history.');
