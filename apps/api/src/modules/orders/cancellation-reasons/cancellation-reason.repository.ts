...
export const cancellationReasonRepository = {
  ...
  findById(tenantId: string, id: string) {
    return db.query.cancellationReasons.findFirst({
      where: and(eq(cancellationReasons.tenantId, tenantId), eq(cancellationReasons.id, id)),
    });
  },
  ...
  async create(tenantId: string, label: string) {
    const [row] = await db.insert(cancellationReasons).values({ tenantId, label }).returning();
    return row;
  },
  async update(tenantId: string, id: string, patch: { label?: string; isActive?: boolean }) {
    const [row] = await db.update(cancellationReasons)
