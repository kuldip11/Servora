...
const activeAt = (promotion: PromotionRow, asOf: Date) => {
  ...
  return (
    (!promotion.startTime || time >= promotion.startTime) &&
    (!promotion.endTime || time <= promotion.endTime)
  );
};

export const allocateCents = (total: number, weights: number[]) => {
...
const addPromotionAttribution = (
  line: PricedLine,
  promotion: PromotionRow,
  amount: number,
) => {
  if (amount <= 0) return;
  line.pricingAttribution.PROMOTION =
    (line.pricingAttribution.PROMOTION ?? 0) - amount;
...
export const applyPromotionStage = async (
  ...
  const matching = candidates
    .filter((promotion) => {
      ...
      return (
        promotion.couponCode.toUpperCase() === requestedCode ||
        requestedIds.has(promotion.id)
      );
    })
  ...
  if (!options.skipSelectionValidation) {
    if (
      requestedCode &&
      !matching.some(
        (promotion) => promotion.couponCode?.toUpperCase() === requestedCode,
      )
    ) {
    ...
    for (const id of requestedIds)
      if (
        !excludedIds.has(id) &&
        !matching.some((promotion) => promotion.id === id)
      )
        throw new ValidationError("Selected promotion is invalid or inactive");
  }
  ...
  for (const promotion of matching) {
    if (promotion.ruleType === "BOGO") {
      ...
      const rewardCategoryId =
        promotion.rewardCategoryId ??
        (promotion.rewardMenuItemId ? null : triggerCategoryId);
      const triggerQuantity = promotion.triggerQuantity ?? 0;
      const rewardQuantity = promotion.rewardQuantity ?? 0;
      const rewardDiscountPercent = Number(
        promotion.rewardDiscountPercent ?? 0,
      );
      if (
        (!triggerItemId && !triggerCategoryId) ||
        triggerQuantity < 1 ||
        rewardQuantity < 1 ||
      ...
      const triggerIndexes = lines.flatMap((line, index) =>
        selectorMatches(line, categoryByItem, triggerItemId, triggerCategoryId)
          ? [index]
          : [],
      );
      const rewardIndexes = lines.flatMap((line, index) =>
        selectorMatches(line, categoryByItem, rewardItemId, rewardCategoryId)
          ? [index]
          : [],
      );
      ...
      while (groupCount > 0) {
        const overlapNeededForTrigger = Math.max(
          0,
          triggerQuantity * groupCount - triggerOnlyUnits,
        );
        const overlapNeededForReward = Math.max(
          0,
          rewardQuantity * groupCount - rewardOnlyUnits,
        );
        if (overlapNeededForTrigger + overlapNeededForReward <= overlapUnits)
          break;
        groupCount -= 1;
      }
      ...
      const allRewardUnits = rewardIndexes.flatMap((lineIndex) => {
        ...
        const remainingLineCents = Math.max(
          0,
          Math.round(line.subtotal * 100) -
            Math.round(discounts[lineIndex]! * 100),
        );
      ...
      const rewardUnits = allRewardUnits
        .filter(
          (unit) =>
            !reservedForTrigger.has(`${unit.lineIndex}:${unit.unitIndex}`),
        )
        .sort(
          (a, b) =>
            a.cents - b.cents ||
      ...
      for (const unit of rewardUnits.slice(0, rewardUnitsToDiscount)) {
        const discountCents = Math.min(
          unit.cents,
          Math.round((unit.cents * rewardDiscountPercent) / 100),
        );
        perLineCents.set(
          unit.lineIndex,
          (perLineCents.get(unit.lineIndex) ?? 0) + discountCents,
        );
      }
      let applied = 0;
      ...
      if (applied > 0)
        redemptionAmounts.set(
          promotion.id,
          (redemptionAmounts.get(promotion.id) ?? 0) + applied,
        );
      continue;
    }
    ...
    const eligibleIndexes = lines.flatMap((line, index) => {
      ...
      if (
        ...
        line.menuItemId !== promotion.scopeMenuItemId
      )
        return [];
      if (
        promotion.scope === "CATEGORY" &&
        categoryByItem.get(line.menuItemId) !== promotion.scopeCategoryId
      )
        return [];
      return [index];
    });
    ...
    const remainingCents = eligibleIndexes.map((index) =>
      Math.max(
        0,
        Math.round(lines[index]!.subtotal * 100) -
          Math.round(discounts[index]! * 100),
      ),
    );
    const remainingTotal = remainingCents.reduce(
      (sum, value) => sum + value,
      0,
    );
    if (!remainingTotal) continue;
    const wanted =
      promotion.ruleType === "PERCENTAGE"
            ...
            Math.max(
              ...
              Math.round(
                (Number(promotion.value) -
                  (options.priorRedemptions?.[promotion.id] ?? 0)) *
                  100,
              ),
            ),
          );
  ...
  return {
    ...
    discountAmount,
    taxAmount,
    redemptions: [...redemptionAmounts].map(
    ...
    appliedPromotions: [...redemptionAmounts].map(
      ([promotionId, discountAmount]) => {
        const promotion = matching.find(
