import type { PricedLine, PricingAttribution, PricingContext, PricingReplayEvidence } from "./pricing/pricing-pipeline";
...
function stableAttribution(item: StoredOrderLineForRepricing): PricingAttribution {
...
export function storedOrderLineToStage4Snapshot(item: StoredOrderLineForRepricing): PricedLine {
  return {
    ...
    ...(item.weightQuantity != null ? { weightQuantity: Number(item.weightQuantity) } : {}),
    ...(item.weightUnit ? { weightUnit: item.weightUnit } : {}),
    ...(item.manualPrice != null ? { manualPrice: Number(item.manualPrice) } : {}),
    ...
    ...(item.pricingReplayEvidence ? { pricingReplayEvidence: item.pricingReplayEvidence } : {}),
    ...
    ...(item.comboSlotOptionId ? { comboSlotOptionId: item.comboSlotOptionId } : {}),
  };
}
...
export async function finalizeWholeActiveOrder(
  ...
  const existingPricingUpdates: ExistingLinePricingUpdate[] = existingItems.map((item, index) => ({
    id: item.id,
