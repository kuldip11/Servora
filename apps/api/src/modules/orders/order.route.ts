...
export const ordersRouter = new Elysia()
  ...
  .get('/api/orders/', ({ auth, query }) => orderController.list(auth, query), { query: orderListQuery })
  .get('/api/orders/:id', ({ auth, params }) => orderController.getById(auth, params.id), {
  ...
  .post(
    '/api/orders/:id/items',
    ({ auth, params, body }) => orderController.fireTicket(auth, params.id, body),
