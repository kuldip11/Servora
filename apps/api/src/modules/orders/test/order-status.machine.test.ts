...
import { ORDER_STATUS_TRANSITIONS, assertValidOrderTransition, canTransitionOrder } from '../order-status.machine';
...
describe('order status machine', () => {
  ...
  it('rejects invalid and terminal-state transitions with domain details', () => {
    ...
    expect(() => assertValidOrderTransition('CLOSED', 'OPEN')).toThrow('Cannot transition order');
    expect(() => assertValidOrderTransition('OPEN', 'PAID')).toThrow('Cannot transition order');
  });
});
