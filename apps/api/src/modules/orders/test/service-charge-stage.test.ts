...
import { calculateTaxServiceAndRounding } from "@/modules/orders/pricing/final-totals-stage";
...
const settings = (
  overrides: Partial<Parameters<typeof calculateTaxServiceAndRounding>[1]> = {},
) => ({
...
describe("pricing pipeline stage 8 service charge", () => {
  it("adds a non-taxable service charge after discounts without changing tax base", () => {
    const item = line(100, 10);
    item.pricingAttribution.PROMOTION = -20;
  ...
  it("folds a taxable service charge into the applicable line tax base", () => {
    const result = calculateTaxServiceAndRounding(
      [line(100, 10)],
      settings({ serviceChargePercent: "5", serviceChargeTaxable: true }),
    );
  ...
  it("taxes a service charge correctly when the merchandise price is tax-inclusive", () => {
    const result = calculateTaxServiceAndRounding(
      ...
      settings({ serviceChargePercent: "10", serviceChargeTaxable: true }),
    );
...
describe("pricing pipeline stage 9 rounding", () => {
  it.each([
    ...
    "applies %s explicitly",
    (roundingPolicy, subtotal, total, adjustment) => {
      const result = calculateTaxServiceAndRounding(
        [line(subtotal, 0)],
...
describe("pricing pipeline stage 7 tax mode", () => {
  ...
  it("handles inclusive and exclusive lines independently in the same order", () => {
    const result = calculateTaxServiceAndRounding(
      [line(110, 10, "INCLUSIVE"), line(100, 10, "EXCLUSIVE")],
      settings(),
    );
