...
const { findByIds, findPricingOverrides, findCandidates, findPerCoverRule } =
  vi.hoisted(() => ({
    findByIds: vi.fn(),
...
import { priceCombo } from "@/modules/menu/combos/combo-pricing";
...
const menu = (overrides: Partial<PricableMenuItem> = {}): PricableMenuItem => ({
  id: "m1",
  branchId: null,
  name: "Pizza",
  categoryId: "cat-1",
...
describe("PricingPipeline stages", () => {
  ...
  it("stage 2 can resolve an unavailable variant for read-only reporting without changing order defaults", () => {
    ...
    expect(() => resolveVariantStage(item, 100, "large")).toThrow(
      "Large is unavailable",
    );
    ...
    const item = menu({
      variants: [
        { id: "small", name: "Small", price: "100", status: "ACTIVE" },
        {
          ...
          manualOverrideReason: "Large packaging unavailable",
        },
      ],
    });
    expect(() => resolveVariantStage(item, 100, "large")).toThrow(
      "Large is unavailable: Large packaging unavailable",
    );
    expect(resolveVariantStage(item, 100, "small")).toMatchObject({
      price: 100,
      variantName: "Small",
    });
  });
  ...
  it("stage 3 validates modifier rules and attributes additive modifier value", () => {
    ...
    expect(
      resolveModifierStage(item, [{ optionId: "o1", quantity: 3 }]),
    ).toEqual({
      modifiers: [
        {
          modifierId: "o1",
  ...
  it("rejects a dependent modifier selection without its prerequisite", () => {
    ...
    expect(() =>
      resolveModifierStage(item, [{ optionId: "cheese", quantity: 1 }]),
    ).toThrow("requires its prerequisite");
    expect(() =>
      resolveModifierStage(item, [
        { optionId: "half", quantity: 1 },
  ...
  it("nets and labels a negative substitution price", () => {
            ...
            options: [
              {
                id: "salad",
                name: "Salad",
                replacesDefaultComponent: "Fries",
                isAvailable: true,
                maxQuantity: 1,
                additionalPrice: "-20",
              },
    ...
    expect(
      resolveModifierStage(item, [{ optionId: "salad", quantity: 1 }]),
    ).toMatchObject({
      attribution: -20,
      modifiers: [{ name: "~~Fries~~ → Salad", price: -20 }],
    });
  });
});
...
describe("PricingPipeline golden regression matrix", () => {
  ...
  it("H1 replays the exact pricing pipeline from immutable evidence without reading current repositories", async () => {
    ...
    const original = await pricingPipeline.price(context, [
    ...
    findByIds.mockRejectedValue(
      new Error("current menu must not be read during replay"),
    );
    findPricingOverrides.mockRejectedValue(
      new Error("current overrides must not be read during replay"),
    );
    findCandidates.mockRejectedValue(
      new Error("current price rules must not be read during replay"),
    );
    ...
    expect(replayed.lines[0]).toMatchObject({
      unitPrice: 120,
      subtotal: 240,
      taxRate: 12,
    });
  ...
  it("matches pre-A4 variant replacement plus modifier pricing", async () => {
    ...
    expect(result.lines[0]!.modifiers[0]).toMatchObject({
      quantity: 2,
      price: 20,
    });
  });
  ...
  it("applies a percent-off price rule only inside its fixed asOf time window", async () => {
    ...
    const active = await pricingPipeline.price(context, [
      { menuItemId: "m1", quantity: 1 },
    ]);
    expect(active.lines[0]).toMatchObject({ unitPrice: 80, subtotal: 80 });

    const inactive = await pricingPipeline.price(
      { ...context, asOf: new Date("2026-08-29T12:00:00.000Z") },
      [{ menuItemId: "m1", quantity: 1 }],
    );
  ...
  it("keeps a winning variant-specific absolute PriceRule through stage 2", async () => {
    findByIds.mockResolvedValue([
      menu({ variants: [{ id: "v1", name: "Large", price: "150" }] }),
    ]);
    findCandidates.mockResolvedValue([
      {
        id: "variant-rule",
  ...
  it("still lets stage 2 replace a branch price override when a lower-specificity rule does not win stage 1", async () => {
    ...
    findPricingOverrides.mockResolvedValue([
      { menuItemId: "m1", price: "120", taxRate: "5" },
    ]);
    findCandidates.mockResolvedValue([
      {
        id: "channel-rule",
...
describe("D2 combo stage consumes authoritative D1 component pricing", () => {
  it("uses an active happy-hour component price before applying the stage-4 combo policy", async () => {
    ...
    const component = await pricingPipeline.price(happyContext, [
      { menuItemId: "m1", quantity: 1 },
    ]);
        ...
        slots: [
          {
            ...
            options: [
              {
                id: "pizza",
                basePrice: component.lines[0]!.unitPrice,
                upcharge: 0,
              },
            ],
          },
...
describe("PricingPipeline D1: full specificity matrix across all dimensions", () => {
  ...
  it("a channel-only rule and a time-window-only rule for the same item resolve independently", () => {
    ...
    expect(selectPriceRule([deliveryMarkup, happyHour], deliveryCtx)?.id).toBe(
      "delivery",
    );
    ...
    expect(selectPriceRule([deliveryMarkup, happyHour], happyHourCtx)?.id).toBe(
      "happy-hour",
    );
  });
  ...
  it("exhaustively prefers every more-specific combination across variant, branch, channel, fulfillment, and time dimensions", () => {
    ...
    const makeScopedRule = (id: string, mask: number): MatchingPriceRule => {
      ...
      if (mask & 16) {
        scoped.startTime = "09:00:00";
        scoped.endTime = "23:00:00";
      }
      return scoped;
    };
    ...
    for (let mask = 1; mask < 1 << dimensions.length; mask += 1) {
      ...
      expect(selectPriceRule([fallback, winner], context, "v1")?.id).toBe(
        winner.id,
      );
    }
  });
  ...
  it("falls back to the next-most-specific rule when the top candidate's variant doesn't match", () => {
    const rules = [
      ...
      rule({
        id: "fully-specific-other-variant",
        variantId: "v2",
        branchId: "b1",
        channel: "STAFF",
        fulfillmentType: "DINE_IN",
  ...
  it("uses start-inclusive/end-exclusive time boundaries so adjacent rules never both match", () => {
    ...
    expect(
      selectPriceRule([first, second], {
        ...context,
        asOf: new Date("2026-08-29T15:59:59.000Z"),
      }),
    ).toBeUndefined();
    expect(
      selectPriceRule([first, second], {
        ...context,
        asOf: new Date("2026-08-29T16:00:00.000Z"),
      })?.id,
    ).toBe("first");
    expect(
      selectPriceRule([first, second], {
        ...context,
        asOf: new Date("2026-08-29T17:59:59.000Z"),
      })?.id,
    ).toBe("first");
    expect(
      selectPriceRule([first, second], {
        ...context,
        asOf: new Date("2026-08-29T18:00:00.000Z"),
      })?.id,
    ).toBe("second");
  });
  ...
  it("resolves an overnight happy-hour window and rejects a non-overlapping time correctly", () => {
    ...
    expect(
      selectPriceRule([overnight], {
        ...context,
        asOf: new Date("2026-08-29T23:30:00.000Z"),
      })?.id,
    ).toBe("late-night");
    expect(
      selectPriceRule([overnight], {
...
describe("PricingPipeline validation regressions", () => {
  it("allows read-only pricing reports to price an unavailable item through the same pipeline", async () => {
    ...
    await expect(
      pricingPipeline.price({ ...context, allowUnavailable: true }, [
        { menuItemId: "m1", quantity: 1 },
      ]),
...
describe("advanced pricing acceptance", () => {
  const modifierItem = (
    zonePricingRule: "AVERAGE" | "HIGHER" | "SUM_HALF" = "HIGHER",
  ) =>
    menu({
      supportsZones: true,
      zonePricingRule,
            ...
            options: [
              {
                ...
                maxQuantity: 1,
                additionalPrice: "20",
                variantPrices: [{ variantId: "large", additionalPrice: "40" }],
              },
              {
                id: "mushroom",
                name: "Mushroom",
  ...
  it.each([
    ...
    expect(priced.modifiers.map((entry) => entry.zoneLabel)).toEqual([
      "LEFT",
      "RIGHT",
    ]);
  });
  ...
  it("G3 prices FIXED, WEIGHT_BASED, and OPEN modes through the same pipeline", async () => {
    findByIds.mockResolvedValue([
      menu({ pricingMode: "FIXED", basePrice: "10" }),
    ]);
    const fixed = await pricingPipeline.price(context, [
      { menuItemId: "m1", quantity: 2 },
    ]);
    expect(fixed).toMatchObject({ subtotal: 20, lines: [{ unitPrice: 10 }] });

    findByIds.mockResolvedValue([
      menu({ pricingMode: "WEIGHT_BASED", weightUnit: "KG", basePrice: "10" }),
    ]);
    const weighted = await pricingPipeline.price(context, [
      { menuItemId: "m1", quantity: 1, weightQuantity: 0.45 },
    ]);
    expect(weighted).toMatchObject({
      subtotal: 4.5,
      lines: [{ unitPrice: 4.5, weightQuantity: 0.45, weightUnit: "KG" }],
    });
    ...
    const open = await pricingPipeline.price(context, [
      { menuItemId: "m1", quantity: 1, manualPrice: 125 },
    ]);
    expect(open).toMatchObject({
      subtotal: 125,
      lines: [{ unitPrice: 125, manualPrice: 125 }],
    });
    await expect(
      pricingPipeline.price(context, [
        { menuItemId: "m1", quantity: 1, manualPrice: 5 },
      ]),
    ).rejects.toThrow("sanity band");
  });
  ...
  it("G7 prefers tenant pricing over organization pricing while allowing org fallback", () => {
    ...
    const tenant: MatchingPriceRule = {
      ...org,
      id: "tenant",
      tenantId: "t1",
      organizationId: null,
  ...
  it("G8 includes customerGroupId in specificity without changing ungrouped resolution", () => {
    ...
    expect(
      selectPriceRule([base, group], { ...context, customerGroupId: "vip" })
        ?.id,
    ).toBe("vip");
  });
});
