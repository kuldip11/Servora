...
const { list, getById, getInventoryImpact, create, updateStatus, fireTicket } =
  vi.hoisted(() => ({
    list: vi.fn(),
    getById: vi.fn(),
...
describe("order controller", () => {
  it("wraps list, detail, and inventory responses", async () => {
    ...
    await expect(
      orderController.getInventoryImpact(auth, "o1"),
    ).resolves.toEqual({ success: true, data: [{ item: "x" }] });
  });
  it("uses created response for creation and delegates mutations", async () => {
    create.mockResolvedValue({ id: "o1" });
    updateStatus.mockResolvedValue({ id: "o1", status: "PAID" });
    fireTicket.mockResolvedValue({ id: "o1" });
    await expect(
      orderController.create(auth, { type: "TAKEAWAY", items: [] }),
    ).resolves.toEqual({ success: true, data: { id: "o1" } });
    await expect(
      orderController.updateStatus(auth, "o1", "PAID", "paid"),
    ).resolves.toEqual({ success: true, data: { id: "o1", status: "PAID" } });
    await expect(
      orderController.fireTicket(auth, "o1", { items: [] }),
    ).resolves.toEqual({ success: true, data: { id: "o1" } });
  });
});
