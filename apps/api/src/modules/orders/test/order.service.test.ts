...
const { findById, findMany, create, updateStatus, fireNewTicket, refireItem, voidItem, compItem, mergeOrders } = vi.hoisted(
  () => ({
    findById: vi.fn(),
    findMany: vi.fn(),
    create: vi.fn(),
...
const { branchFind, tableFind, tableOpen, tableUpdate, transferOrderTable } = vi.hoisted(() => ({
  branchFind: vi.fn(),
  tableFind: vi.fn(),
  tableOpen: vi.fn(),
  tableUpdate: vi.fn(),
...
const { promotionFindCandidates, listRedemptionsForOrder, writeAudit } = vi.hoisted(() => ({
...
vi.mock("../order.repository", () => ({
  orderRepository: { findById, findMany, create, updateStatus, fireNewTicket, refireItem, voidItem, compItem, mergeOrders },
}));
...
vi.mock("../../menu/pricing/price-rule.repository", () => ({
  priceRuleRepository: { findCandidates: priceRuleCandidates, findPerCoverRule: perCoverRule },
}));
vi.mock("../../kitchen-tickets/ticket.repository", () => ({
  ticketRepository: { allServed, findDetailedById: findDetailedTicket, shouldHoldCourse, hasCourseNumber },
}));
...
vi.mock("../../menu/promotions/promotion.repository", () => ({
  promotionRepository: { findCandidates: promotionFindCandidates, listRedemptionsForOrder },
}));
...
beforeEach(() => {
  vi.resetAllMocks();
  tenantFind.mockResolvedValue({ serviceChargePercent: null, serviceChargeTaxable: false, roundingPolicy: "NONE", defaultTaxMode: "EXCLUSIVE" });
  ...
  effectiveWithEvidence.mockResolvedValue({
    ...
    evidence: {
      item: { id: "m1", branchId: null, status: "ACTIVE", basePrice: "10", taxRate: "5", prepTimeMinutes: null, availabilityReason: null, manualOverrideStatus: null, manualOverrideReason: null, manualStockCount: null },
...
describe("order service", () => {
  ...
  it("keeps normal orders out of course mode and requires tenant opt-in for explicit courses", async () => {
    ...
    findById.mockResolvedValue({ ...order, id: "o1", kitchenTickets: [{ id: "kt1", branchId: "b1", status: "FIRED", items: [{ id: "i1", menuItemId: "m1", variantId: null, quantity: 1, weightQuantity: null, weightUnit: null, modifiers: [] }] }] });
    await orderService.create(auth(), { type: "TAKEAWAY", items: [{ menuItemId: "m1", quantity: 1 }] });
    expect(create).toHaveBeenLastCalledWith(expect.objectContaining({
      items: [expect.not.objectContaining({ courseNumber: expect.anything() })],
    }));

    await expect(orderService.create(auth(), {
      type: "TAKEAWAY", items: [{ menuItemId: "m1", quantity: 1, courseNumber: 1 }],
    })).rejects.toThrow("Course sequencing is not enabled");

    tenantFind.mockResolvedValue({
      serviceChargePercent: null, serviceChargeTaxable: false, roundingPolicy: "NONE", defaultTaxMode: "EXCLUSIVE",
  ...
  it("creates an order, publishes events, and treats inventory deduction as best-effort", async () => {
    ...
    findById.mockResolvedValue({
      ...order,
      id: "o1",
      kitchenTickets: [{ id: "kt1", branchId: "b1", status: "FIRED", items: [{ id: "i1", menuItemId: "m1", variantId: null, quantity: 1, weightQuantity: null, weightUnit: null, modifiers: [] }] }],
    });
  ...
  it("persists combo parent + children and deducts inventory only for real components", async () => {
    ...
    priceCombos.mockResolvedValue({
      lines: [
        {
          menuItemId: null, menuItemName: "Lunch Combo", quantity: 1, unitPrice: 8, subtotal: 0, taxRate: 0,
          fulfillmentType: "TAKEAWAY", modifiers: [],
          pricingAttribution: { BASE_PRICE: 0, VARIANT: 0, MODIFIER: 0, COMBO: 8 },
          comboId: "combo-1", comboGroupId: groupId,
        },
        {
          menuItemId: "m1", menuItemName: "Pizza", quantity: 1, unitPrice: 8, subtotal: 8, taxRate: 5,
          fulfillmentType: "TAKEAWAY", modifiers: [],
          pricingAttribution: { BASE_PRICE: 10, VARIANT: 0, MODIFIER: 0, COMBO: -2 },
          comboId: "combo-1", comboGroupId: groupId,
        },
      ],
      subtotal: 8,
    ...
    await orderService.create(auth(), {
      type: "TAKEAWAY",
      items: [],
      combos: [{ comboId: "combo-1", selections: [{ slotId: "slot-1", optionIds: ["option-1"] }] }],
    });
    ...
    expect(create).toHaveBeenCalledWith(expect.objectContaining({
      ...
      taxAmount: 0.4,
      items: [
        expect.objectContaining({ menuItemId: null, comboId: "combo-1", comboGroupId: groupId, stationId: null, menuChangeEventId: null }),
        expect.objectContaining({ menuItemId: "m1", comboId: "combo-1", comboGroupId: groupId, stationId: "station-1", menuChangeEventId: "event-1" }),
      ],
    }));
    expect(latestForItems).toHaveBeenCalledWith("t1", ["m1"], expect.any(Date));
    expect(resolveStation).toHaveBeenCalledTimes(1);
    expect(deduct).toHaveBeenCalledWith("t1", "b1", "o1", "kt1", [
  ...
  it("only fires additional tickets from OPEN orders", async () => {
    ...
    expect(effective).toHaveBeenLastCalledWith(
      "t1",
      "m1",
      "b1",
      {
  ...
  it("refires a combo component from its stored allocation, refreshes routing, and performs a fresh inventory deduction", async () => {
    const original = {
      id: "combo-child", menuItemId: "m1", menuItemName: "Pizza", variantId: null, variantName: null,
      quantity: 1, unitPrice: "8.00", subtotal: "8.00", taxRate: "5", taxMode: "EXCLUSIVE" as const,
      fulfillmentType: "DINE_IN" as const, itemStatus: "ACTIVE", comboId: "combo-1", comboGroupId: "group-1",
      stationId: "old-station", menuChangeEventId: "old-event", pricingAttribution: { BASE_PRICE: 10, VARIANT: 0, MODIFIER: 0, COMBO: -2 },
      modifiers: [], compedAt: null,
    };
    const fullBefore = {
      ...order, items: [original], customerId: null,
      kitchenTickets: [{ id: "kt-original", status: "PREPARING", items: [{ id: original.id }] }],
    };
    const fullAfter = { ...order, items: [
      { ...original, itemStatus: "REFIRED", compedAt: new Date() },
      { ...original, id: "replacement", itemStatus: "ACTIVE", stationId: "station-1", menuChangeEventId: "event-1", refiresOrderItemId: original.id },
    ] };
    findById.mockResolvedValueOnce(fullBefore).mockResolvedValueOnce(fullAfter);
    refireItem.mockResolvedValue({ ticket: { id: "kt-refire" }, replacement: { id: "replacement" } });
    findDetailedTicket
      .mockResolvedValueOnce({
        id: "kt-refire", orderId: "o1", branchId: "b1", status: "FIRED",
        items: [{ id: "replacement", menuItemId: "m1", variantId: null, quantity: 1, modifiers: [] }],
      })
      .mockResolvedValueOnce({
        id: "kt-original", orderId: "o1", branchId: "b1", status: "PREPARING",
        items: [{ ...original, itemStatus: "REFIRED" }],
      });
    deduct.mockResolvedValue({ short: [] });

    await expect(orderService.refireItem(auth(), "o1", original.id, "burnt", true)).resolves.toEqual(fullAfter);

    expect(refireItem).toHaveBeenCalledWith(expect.objectContaining({
      originalItemId: original.id, compOriginal: true,
      item: expect.objectContaining({
        menuItemId: "m1", comboId: "combo-1", comboGroupId: "group-1",
        unitPrice: 8, subtotal: 8, stationId: "station-1", menuChangeEventId: "event-1",
      }),
    }));
    expect(resolveStation).toHaveBeenCalledWith("t1", "m1", []);
    expect(deduct).toHaveBeenCalledWith("t1", "b1", "o1", "kt-refire", [
      expect.objectContaining({ orderItemId: "replacement", menuItemId: "m1", quantity: 1 }),
    ], "u1");
    expect(writeAudit).toHaveBeenCalledWith(expect.objectContaining({ action: "ORDER_ITEM_REFIRED" }));
    expect(publish).toHaveBeenCalledWith(
      expect.objectContaining({ type: "kitchen.ticket.updated", payload: expect.objectContaining({ id: "kt-original" }) }),
      "t1", "b1",
    );
    expect(publish).toHaveBeenCalledWith(
      expect.objectContaining({ type: "kitchen.ticket.created", payload: expect.objectContaining({ id: "kt-refire" }) }),
      "t1", "b1",
    );
  });

  it.each(["HELD", "PENDING_PAYMENT"])("rejects refire when the original ticket is %s and was never sent to kitchen", async (status) => {
    const original = {
      id: "i-held", menuItemId: "m1", menuItemName: "Pizza", variantId: null, variantName: null,
      quantity: 1, unitPrice: "10.00", subtotal: "10.00", taxRate: "5", taxMode: "EXCLUSIVE",
      fulfillmentType: "DINE_IN", itemStatus: "ACTIVE", pricingAttribution: null, modifiers: [], compedAt: null,
    };
    findById.mockResolvedValue({
      ...order, items: [original], customerId: null,
      kitchenTickets: [{ id: "kt-held", status, items: [{ id: original.id }] }],
    });
    await expect(orderService.refireItem(auth(), "o1", original.id, "redo", false))
  ...
  it("rejects orphan later courses whose immediate predecessor does not exist", async () => {
    findById.mockResolvedValue({ ...order, items: [], customerId: null, kitchenTickets: [] });
    tenantFind.mockResolvedValue({
      serviceChargePercent: null, serviceChargeTaxable: false, roundingPolicy: "NONE", defaultTaxMode: "EXCLUSIVE",
      courseSequencingEnabled: true,
    });
    hasCourseNumber.mockResolvedValue(false);
    await expect(orderService.fireTicket(auth(), "o1", {
  ...
  it("voids an unserved active line and recomputes totals", async () => {
    ...
    voidItem.mockResolvedValue({ voided: { id: "i1", itemStatus: "VOIDED" }, reversedInventoryItemIds: ["inv1"] });

    await expect(orderService.voidItem(auth(), "o1", "i1", "mistake"))
      .resolves.toMatchObject({ totalAmount: "22" });
    expect(voidItem).toHaveBeenCalledWith(
      "t1", "o1", "i1", "u1", "mistake", undefined,
      expect.objectContaining({ subtotal: 20, discountAmount: 0, taxAmount: 2, serviceChargeAmount: 0, roundingAdjustment: 0, totalAmount: 22 }),
    );
    expect(publish).toHaveBeenCalledWith(
  ...
  it("voiding any combo member removes the entire combo group from totals", async () => {
    ...
    const detailed = {
      ...
      discountAmount: "0",
      items: [
        { id: "parent", comboGroupId: groupId, itemStatus: "ACTIVE", subtotal: "0", taxRate: "0" },
        { id: "child", comboGroupId: groupId, itemStatus: "ACTIVE", subtotal: "8", taxRate: "5" },
        { id: "regular", comboGroupId: null, itemStatus: "ACTIVE", subtotal: "20", taxRate: "10" },
      ],
      kitchenTickets: [{ id: "kt1", status: "FIRED", items: [{ id: "parent" }, { id: "child" }, { id: "regular" }] }],
    };
    findById.mockResolvedValueOnce(detailed).mockResolvedValueOnce({
      ...detailed, subtotal: "20", taxAmount: "2", totalAmount: "22",
    });
    ...
    expect(voidItem).toHaveBeenCalledWith(
      "t1", "o1", "parent", "u1", "cancel combo", undefined,
      expect.objectContaining({ subtotal: 20, discountAmount: 0, taxAmount: 2, serviceChargeAmount: 0, roundingAdjustment: 0, totalAmount: 22 }),
    );
  });
  ...
  it("comps a served line without reversing its inventory deduction", async () => {
    ...
    await expect(orderService.compItem(auth(), "o1", "i1", "service recovery"))
      .resolves.toMatchObject({ totalAmount: "22" });
    expect(compItem).toHaveBeenCalledWith(
      "t1", "o1", "i1", "u1", "service recovery", undefined,
      expect.objectContaining({ subtotal: 20, discountAmount: 0, taxAmount: 2, serviceChargeAmount: 0, roundingAdjustment: 0, totalAmount: 22 }),
    );
  ...
  it("requires the distinct comp permission", async () => {
    await expect(
      orderService.compItem(auth({ permissions: ["orders:void"] }), "o1", "i1", "reason"),
    ).rejects.toThrow("Insufficient permissions");
  });
  ...
  it("cancels with structured, free-text, or combined reasons", async () => {
    for (const [reason, reasonId] of [
      ...
      findById.mockResolvedValueOnce(order).mockResolvedValueOnce({ ...order, status: "CANCELLED" });
      updateStatus.mockResolvedValue({ ...order, status: "CANCELLED" });
      await expect(orderService.updateStatus(auth(), "o1", "CANCELLED", reason, reasonId))
        .resolves.toMatchObject({ status: "CANCELLED" });
      expect(updateStatus).toHaveBeenLastCalledWith(
        "t1", "o1", "CANCELLED", "u1", reason, reasonId, "b1",
      );
    }
  ...
  it("atomically transfers an open dine-in order and publishes floor and kitchen updates", async () => {
    ...
    tableFind.mockImplementation((_tenant: string, id: string) => Promise.resolve(
      id === "old"
        ? { id, branchId: "b1", name: "Table 1", status: "OCCUPIED", isActive: true }
        : { id, branchId: "b1", name: "Table 2", status: "AVAILABLE", isActive: true },
    ));
    ...
    findDetailedTicket.mockResolvedValue({ id: "kt1", status: "FIRED", order: { table: { name: "Table 2" } } });

    await expect(orderService.transferTable(auth(), "o1", "new", "Guest request"))
      .resolves.toMatchObject({ tableId: "new" });
    expect(transferOrderTable).toHaveBeenCalledWith(expect.objectContaining({
      orderId: "o1",
      oldTableId: "old",
      newTableId: "new",
      customerSessionId: "session-1",
    ...
    expect(publish).toHaveBeenCalledWith(
      expect.objectContaining({ type: "kitchen.ticket.updated" }), "t1", "b1",
    );
  });
  ...
  it("rejects unavailable and concurrently claimed transfer destinations", async () => {
    ...
    tableFind.mockImplementation((_tenant: string, id: string) => Promise.resolve(
      id === "old"
        ? { id, branchId: "b1", name: "Table 1", status: "OCCUPIED", isActive: true }
        : { id, branchId: "b1", name: "Table 2", status: "OCCUPIED", isActive: true },
    ));
    await expect(orderService.transferTable(auth(), "o1", "new"))
      .rejects.toThrow("destination table is not available");
    tableFind.mockImplementation((_tenant: string, id: string) => Promise.resolve(
      { id, branchId: "b1", name: id, status: id === "new" ? "AVAILABLE" : "OCCUPIED", isActive: true },
    ));
  ...
  it("rejects closed or already-merged table merges with a clear domain error", async () => {
    ...
    await expect(orderService.mergeOrders(auth(), "source", "target")).rejects.toThrow("cannot be merged");
    mergeOrders.mockResolvedValueOnce({ status: "already_merged" });
    await expect(orderService.mergeOrders(auth(), "source", "target")).rejects.toThrow("merge chain");
  });

  it("merges billing without changing kitchen tickets", async () => {
    mergeOrders.mockResolvedValue({ status: "ok", source: { id: "source" }, target: { id: "target" } });
    findById.mockImplementation((_tenant: string, id: string) => Promise.resolve({ ...order, id, kitchenTickets: [{ id: `${id}-ticket`, status: "FIRED" }] }));
    const beforeTicketCalls = findDetailedTicket.mock.calls.length;
    await expect(orderService.mergeOrders(auth(), "source", "target")).resolves.toMatchObject({ source: { id: "source" }, target: { id: "target" } });
...
describe("Phase G order lifecycle acceptance", () => {
  it("G9 creates PER_COVER orders from the cover rate and applies normal tax/service-charge/rounding stages", async () => {
    ...
    perCoverRule.mockResolvedValue({
      id: "cover-adult", tenantId: "t1", organizationId: null, menuItemId: null, menuItemSku: null,
      variantId: null, branchId: null, channel: null, fulfillmentType: null,
      startDate: null, endDate: null, startTime: null, endTime: null,
      customerGroupId: null, coverTier: null, priority: 0, price: "23", percentOff: null,
      taxRate: "10", isPerCover: true, isActive: true,
    });
    create.mockResolvedValue({ id: "buffet-1" });
    findById.mockResolvedValue({ id: "buffet-1", branchId: "b1", status: "OPEN", billingMode: "PER_COVER", totalAmount: "82.50" });
  ...
  it("G6 produces a zero-priced REFILL sibling, preserves order totals, and deducts inventory again", async () => {
    const original = {
      id: "rice-1", menuItemId: "m1", menuItemName: "Rice", variantId: null, variantName: null,
      quantity: 1, unitPrice: "5.00", subtotal: "5.00", taxRate: "5", taxMode: "EXCLUSIVE" as const,
      fulfillmentType: "DINE_IN" as const, itemStatus: "ACTIVE", comboId: "thali", comboGroupId: "group-1",
      comboSlotOptionId: "slot-option-rice", comboSlotOption: { id: "slot-option-rice", isUnlimitedRefill: true },
      stationId: "station-1", menuChangeEventId: "event-1", pricingAttribution: { BASE_PRICE: 5, VARIANT: 0, MODIFIER: 0, COMBO: 0 },
      modifiers: [], compedAt: null,
    };
    const before = {
      ...order, subtotal: "20.00", discountAmount: "0", taxAmount: "1.00", serviceChargeAmount: "0",
      roundingAdjustment: "0", totalAmount: "21.00", items: [original],
      kitchenTickets: [{ id: "kt-original", status: "PREPARING", items: [{ id: original.id }] }],
    };
    const after = { ...before, items: [...before.items, { ...original, id: "rice-refill", unitPrice: "0.00", subtotal: "0.00", refireType: "REFILL" }] };
    findById.mockResolvedValueOnce(before).mockResolvedValueOnce(after);
    refireItem.mockResolvedValue({ ticket: { id: "kt-refill" }, replacement: { id: "rice-refill" } });
    findDetailedTicket.mockResolvedValue({
      id: "kt-refill", orderId: "o1", branchId: "b1", status: "FIRED",
      items: [{ id: "rice-refill", menuItemId: "m1", variantId: null, quantity: 1, weightQuantity: null, weightUnit: null, modifiers: [] }],
    });
    ...
    expect(refireItem).toHaveBeenCalledWith(expect.objectContaining({
      ...
      item: expect.objectContaining({ unitPrice: 0, subtotal: 0, billingExcluded: true, comboSlotOptionId: "slot-option-rice" }),
      absoluteTotals: expect.objectContaining({ subtotal: 20, taxAmount: 1, totalAmount: 21 }),
    }));
    expect(deduct).toHaveBeenCalledWith("t1", "b1", "o1", "kt-refill", [
      expect.objectContaining({ orderItemId: "rice-refill", menuItemId: "m1", quantity: 1 }),
    ], "u1");
    expect(writeAudit).toHaveBeenCalledWith(expect.objectContaining({ action: "ORDER_ITEM_REFILL" }));
  });
});
