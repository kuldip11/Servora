...
const { findCandidates, findCustomer, findOrganizationTierForCustomer } =
  vi.hoisted(() => ({
...
vi.mock("../../loyalty/loyalty.repository", () => ({
  loyaltyRepository: { findCustomer, findOrganizationTierForCustomer },
}));

import {
  applyDiscountStages,
  applyLoyaltyDiscount,
} from "@/modules/orders/pricing/loyalty-stage";
import type {
  PricedLine,
  PricingContext,
...
const line = (subtotal: number): PricedLine => ({
  menuItemId: "item",
  menuItemName: "Item",
  quantity: 1,
  unitPrice: subtotal,
...
describe("D6 fixed loyalty allocation", () => {
  it("applies a fixed tier discount once across the whole order and preserves exact cents", () => {
    const result = applyLoyaltyDiscount(
      [line(60), line(40)],
      customer(null, "10").loyaltyTier,
    );
    expect(result.loyaltyDiscountAmount).toBe(10);
    expect(
      result.lines.reduce(
        (sum, priced) =>
          sum + Math.max(0, -(priced.pricingAttribution.LOYALTY ?? 0)),
        0,
      ),
    ).toBe(10);
  });
...
describe("pricing pipeline stage 6 loyalty", () => {
  ...
  it("uses an organization-level tier at a sibling tenant when identity resolution finds it", async () => {
    ...
    const result = await applyDiscountStages(context, [line(100)], {
      customerId: "customer",
    });
    expect(result.loyaltyDiscountAmount).toBe(15);
    expect(result.lines[0]!.pricingAttribution.LOYALTY_DETAILS?.tierId).toBe(
      "org-tier",
    );
  });
  ...
  it("stacks a loyalty discount after a stackable promotion", async () => {
    findCandidates.mockResolvedValue([
      promo({ value: "20", stackableWithLoyalty: true }),
    ]);
  ...
  it("keeps the larger non-stackable promotion instead of loyalty", async () => {
    findCandidates.mockResolvedValue([
      promo({ value: "30", stackableWithLoyalty: false }),
    ]);
  ...
  it("drops a smaller non-stackable promotion when loyalty is larger", async () => {
    findCandidates.mockResolvedValue([
      promo({ value: "10", stackableWithLoyalty: false }),
    ]);
    findCustomer.mockResolvedValue(customer("20", null));
    const result = await applyDiscountStages(context, [line(100)], {
      customerId: "customer",
