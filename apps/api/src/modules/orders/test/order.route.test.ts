...
vi.mock('../order.controller', () => ({ orderController: { list: vi.fn(), getById: vi.fn(), getInventoryImpact: vi.fn(), create: vi.fn(), updateStatus: vi.fn(), fireTicket: vi.fn() } }));
import { ordersRouter } from '../order.route';

describe('orders routes', () => {
  it('registers all documented order endpoints', () => {
    expect((ordersRouter as any).routes).toEqual(expect.arrayContaining([
      expect.objectContaining({ method: 'GET', path: '/api/orders/' }),
      expect.objectContaining({ method: 'GET', path: '/api/orders/:id' }),
      expect.objectContaining({ method: 'GET', path: '/api/orders/:id/inventory-impact' }),
      expect.objectContaining({ method: 'POST', path: '/api/orders/' }),
      expect.objectContaining({ method: 'PATCH', path: '/api/orders/:id/status' }),
      expect.objectContaining({ method: 'POST', path: '/api/orders/:id/items' }),
    ]));
  });
});
