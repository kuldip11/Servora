...
import { applyPromotionStage } from "../pricing/promotion-stage";
import type { PricedLine, PricingContext } from "../pricing/pricing-pipeline";
import type { PromotionRow } from "../../menu/promotions/promotion.repository";
...
describe("pricing pipeline stage 5 promotions", () => {
  ...
  it("previews an unsaved promotion through the same stage without consulting persisted candidates", async () => {
    const preview = promotion({ id: "preview", value: "25.00", name: "Unsaved 25%" });
  ...
  it("discounts the post-combo/modifier subtotal and writes line attribution", async () => {
    ...
    const result = await applyPromotionStage(context, [comboChild, line("item-b", 20)]);
    ...
    expect(result.redemptions).toEqual([{ promotionId: "promo-1", customerId: null, discountAmount: 20 }]);
  });

  it("honors ITEM and CATEGORY scopes", async () => {
    findCandidates.mockResolvedValue([
      promotion({ id: "item-promo", name: "item", scope: "ITEM", scopeMenuItemId: "item-a", value: "10" }),
      promotion({ id: "category-promo", name: "category", scope: "CATEGORY", scopeCategoryId: "cat-b", value: "50" }),
    ]);
    const result = await applyPromotionStage(context, [line("item-a", 100), line("item-b", 100)]);
  ...
  it("allocates a fixed amount deterministically without losing a cent", async () => {
    findCandidates.mockResolvedValue([promotion({ ruleType: "FIXED_AMOUNT", value: "1.00" })]);
    const result = await applyPromotionStage(context, [line("item-a", 1), line("item-b", 1), line("item-c", 1)]);
    const discounts = result.lines.map((value) => -(value.pricingAttribution.PROMOTION ?? 0));
  ...
  it("does not reapply an already-consumed fixed order discount on a later round", async () => {
    findCandidates.mockResolvedValue([promotion({ ruleType: "FIXED_AMOUNT", value: "50.00" })]);
    const result = await applyPromotionStage(context, [line("item-a", 100)], { priorRedemptions: { "promo-1": 50 }, promotionIds: ["promo-1"] });
  ...
  it("rejects an invalid requested coupon instead of silently ignoring it", async () => {
    ...
    await expect(applyPromotionStage(context, [line("item-a", 100)], { couponCode: "BAD" })).rejects.toThrow("Coupon code is invalid or inactive");
  });


  it("BOGO buy-2-get-1 requires the full trigger plus reward quantity", async () => {
    findCandidates.mockResolvedValue([promotion({
      ruleType: "BOGO", value: null, name: "Buy 2 get 1",
      triggerMenuItemId: "item-a", rewardDiscountPercent: "100", triggerQuantity: 2, rewardQuantity: 1,
    })]);
    const below = await applyPromotionStage(context, [{ ...line("item-a", 200), quantity: 2, unitPrice: 100 }]);
    expect(below.discountAmount).toBe(0);
    const exact = await applyPromotionStage(context, [{ ...line("item-a", 300), quantity: 3, unitPrice: 100 }]);
    expect(exact.discountAmount).toBe(100);
    const above = await applyPromotionStage(context, [{ ...line("item-a", 400), quantity: 4, unitPrice: 100 }]);
    expect(above.discountAmount).toBe(100);
  });

  it("BOGO never reuses one overlapping unit as both trigger and reward", async () => {
    findCandidates.mockResolvedValue([promotion({
      ruleType: "BOGO", value: null, name: "Buy pizza get Margherita",
      triggerCategoryId: "cat-a", rewardMenuItemId: "item-a",
      rewardDiscountPercent: "100", triggerQuantity: 1, rewardQuantity: 1,
    })]);
  ...
  it("BOGO discounts the lowest-priced eligible reward unit first", async () => {
    findCandidates.mockResolvedValue([promotion({
      ruleType: "BOGO", value: null, name: "Buy A get B",
      triggerMenuItemId: "item-a", rewardMenuItemId: "item-b",
      rewardDiscountPercent: "100", triggerQuantity: 1, rewardQuantity: 1,
    })]);
