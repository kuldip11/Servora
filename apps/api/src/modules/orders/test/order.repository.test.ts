...
beforeEach(() => {
  ...
  tx.query = {
    ...
    menuItems: { findFirst: vi.fn().mockResolvedValue({ manualStockCount: null }) },
...
describe("order repository", () => {
  ...
  it("persists and reads promotion attribution on order lines without losing detail", async () => {
    ...
    expect(itemInsert.values).toHaveBeenCalledWith([
      expect.objectContaining({ pricingAttribution: attributedItem.pricingAttribution }),
    ]);

    db.query.orders.findFirst.mockResolvedValue({
      id: "o-attributed",
      tenantId: "t1",
      items: [{ id: "oi-attributed", pricingAttribution: attributedItem.pricingAttribution }],
    });
    await expect(orderRepository.findById("t1", "o-attributed")).resolves.toMatchObject({
  ...
  it("E5 void reverses every persisted multi-source deduction row exactly once", async () => {
    const deductionRows = [
      { id: "d-variant", orderItemId: "oi1", inventoryItemId: "inv-variant", quantityDeducted: "1.250" },
      { id: "d-modifier", orderItemId: "oi1", inventoryItemId: "inv-modifier", quantityDeducted: "0.500" },
      { id: "d-sub-recipe", orderItemId: "oi1", inventoryItemId: "inv-sub", quantityDeducted: "2.000" },
    ];
    tx.query = {
      orderItems: {
        findFirst: vi.fn().mockResolvedValue({ id: "oi1", itemStatus: "ACTIVE", comboGroupId: null }),
        findMany: vi.fn(),
      },
      orderInventoryDeductions: { findMany: vi.fn().mockResolvedValue(deductionRows) },
      inventoryItems: {
        findFirst: vi.fn()
          .mockResolvedValueOnce({ id: "inv-variant", tenantId: "t1", currentStock: "10" })
          .mockResolvedValueOnce({ id: "inv-modifier", tenantId: "t1", currentStock: "20" })
          .mockResolvedValueOnce({ id: "inv-sub", tenantId: "t1", currentStock: "30" }),
      },
        ...
        const chain = {
          where: vi.fn(() => {
            if ("itemStatus" in data) {
              return { returning: vi.fn().mockResolvedValue([{ id: "oi1", itemStatus: "VOIDED" }]) };
            }
    ...
    const result = await orderRepository.voidItem(
      ...
      "mistake",
      undefined,
      { subtotal: 0, discountAmount: 0, taxAmount: 0, serviceChargeAmount: 0, roundingAdjustment: 0, totalAmount: 0 },
    );
    ...
    expect(stockSets.map((entry) => entry.currentStock)).toEqual(["11.250", "20.500", "32.000"]);
    expect(reversalTransactions.map((entry) => entry.reversalOfDeductionId)).toEqual([
      "d-variant", "d-modifier", "d-sub-recipe",
    ]);
  ...
  it("creates later courses as HELD while keeping the first course FIRED", async () => {
    ...
    tx.insert
      ...
      .mockReturnValueOnce({ values: vi.fn().mockResolvedValue(undefined) });
    await orderRepository.create({
      tenantId: "t1", branchId: "b1", createdBy: "u1", type: "DINE_IN",
      items: [{ ...item, courseNumber: 1 }, { ...item, menuItemId: "m2", menuItemName: "Dessert", courseNumber: 2 }],
      subtotal: 20, taxAmount: 3, totalAmount: 23,
    });
    expect(ticket1Insert.values).toHaveBeenCalledWith(expect.objectContaining({ status: "FIRED", courseId: "course-1" }));
    expect(ticket2Insert.values).toHaveBeenCalledWith(expect.objectContaining({ status: "HELD", courseId: "course-2", firedAt: null }));
  });

  it("creates a refire sibling with lineage and can comp the original while keeping it REFIRED", async () => {
    tx.select.mockReturnValue({ from: vi.fn().mockReturnValue({ where: vi.fn().mockResolvedValue([{ maxTicket: 1 }]) }) });
    ...
    tx.update.mockImplementation(() => ({
      set: vi.fn((data: Record<string, unknown>) => {
        ...
        return {
          where: vi.fn(() => {
            updateCall += 1;
            if (updateCall === 1) return { returning: vi.fn().mockResolvedValue([{ id: "oi1" }]) };
            return Promise.resolve(undefined);
          }),
        };
      }),
    ...
    const result = await orderRepository.refireItem({
      tenantId: "t1", branchId: "b1", orderId: "o1", originalItemId: "oi1",
      item: { ...item, comboId: "combo-1", comboGroupId: "group-1", unitPrice: 8, subtotal: 8 }, changedBy: "u1", reason: "burnt", compOriginal: true,
      absoluteTotals: { subtotal: 10, taxAmount: 1.5, discountAmount: 0, serviceChargeAmount: 0, roundingAdjustment: 0, totalAmount: 11.5 },
    });
    expect(result).toEqual({ ticket: { id: "ticket-2" }, replacement: { id: "replacement" } });
    expect(itemInsert.values).toHaveBeenCalledWith(expect.objectContaining({
      refiresOrderItemId: "oi1", comboId: "combo-1", comboGroupId: "group-1", unitPrice: "8.00", subtotal: "8.00",
    }));
    expect(sets).toContainEqual(expect.objectContaining({ itemStatus: "REFIRED", compedAt: expect.any(Date), refireReason: "burnt" }));
  });
...
describe("G4 atomic manual-stock depletion", () => {
  it("rejects the order when the compare-and-decrement loses the last-unit race", async () => {
    tx.query = {
      menuItemVariants: { findFirst: vi.fn().mockResolvedValue(undefined) },
      menuItems: { findFirst: vi.fn().mockResolvedValue({ manualStockCount: 1 }) },
    };
    tx.update.mockReturnValue({
