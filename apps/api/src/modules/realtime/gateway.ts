 ...
 */
export async function resolveRealtimeContext(payload: JwtPayload, tenantId: string, branchId?: string) {
  ...
  if (!decision.allowed || !decision.permissionKeys.some((key) =>
    ['orders:read', 'kitchen:read', 'inventory:read', 'tables:read'].includes(key),
  )) {
    throw new Error('REALTIME_PERMISSION_REQUIRED');
  }

  if (branchId && branchId !== 'all' && !decision.branchIds.includes(branchId) && !decision.tenantWide) {
    throw new Error('REALTIME_BRANCH_ACCESS_REQUIRED');
  }

  return { tenantId: membership.tenantId, membershipId: membership.id, branchId: branchId && branchId !== 'all' ? branchId : null };
}
  ...
  subscriber.on('message', (channel, message) => {
    try {
      ...
      for (const ws of tenantClients) {
        try {
          if (shouldDeliverRealtimeEvent((ws as any).__branchId, event.branchId)) {
            ws.send(message);
          }
        } catch {
...
export const realtimeRouter = new Elysia({ prefix: '/ws' }).ws('/events', {
  async open(ws) {
    ...
    try {
      ...
      const branchId = ws.data.query['branchId'] ? String(ws.data.query['branchId']) : undefined;
      ...
      ws.send(JSON.stringify({ type: 'connected', tenantId: context.tenantId }));
    } catch {
