...
vi.mock('../staff.controller',()=>({staffController:{list:vi.fn(),create:vi.fn(),update:vi.fn(),remove:vi.fn(),listRoles:vi.fn()}}));
import { staffRouter, rolesRouter } from '../staff.route';
describe('staff routes',()=>{it('registers authenticated staff CRUD routes',()=>{const routes=(staffRouter as any).routes;expect(routes).toEqual(expect.arrayContaining([expect.objectContaining({method:'GET',path:'/api/staff/'}),expect.objectContaining({method:'POST',path:'/api/staff/'}),expect.objectContaining({method:'PATCH',path:'/api/staff/:id'}),expect.objectContaining({method:'DELETE',path:'/api/staff/:id'})]));});it('registers the authenticated roles lookup route',()=>{expect((rolesRouter as any).routes).toEqual(expect.arrayContaining([expect.objectContaining({method:'GET',path:'/api/roles/'})]));});});
