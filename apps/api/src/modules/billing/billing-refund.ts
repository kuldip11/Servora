...
export type RefundResolution =
  | { ok: true }
  | { ok: false; reason: 'PAYMENT_NOT_REFUNDABLE' | 'REFUND_AMOUNT_EXCEEDS_PAYMENT' };
...
export function resolveRefundEligibility(
  ...
  if (paymentStatus !== 'SUCCESS') return { ok: false, reason: 'PAYMENT_NOT_REFUNDABLE' };
  if (requestedAmount > paymentAmount) return { ok: false, reason: 'REFUND_AMOUNT_EXCEEDS_PAYMENT' };
  return { ok: true };
}
