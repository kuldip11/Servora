...
export const billingRepository = {
    ...
    return db.transaction(async (tx) => {
      ...
      const billingOrder = order.mergedIntoOrderId
        ? await tx.query.orders.findFirst({
            where: and(
              eq(orders.id, order.mergedIntoOrderId),
              eq(orders.tenantId, data.tenantId),
            ),
          })
        : order;
      ...
      const mergedOrders = await tx.query.orders.findMany({
        where: eq(orders.mergedIntoOrderId, billingOrderId),
      });
      ...
      if (!data.billId && orderBills.length > 1)
        return { status: "bill_required" };
      if (!bill) {
        ...
        const activeItems = await tx.query.orderItems.findMany({
          where: and(
            inArray(orderItems.orderId, combinedOrderIds),
            eq(orderItems.billingExcluded, false),
            or(
              eq(orderItems.itemStatus, "ACTIVE"),
              and(
                eq(orderItems.itemStatus, "REFIRED"),
                isNull(orderItems.compedAt),
              ),
            ),
          ),
        });
      ...
      const existingSuccessfulPayments = await tx
        ...
        .from(payments)
        .where(
          and(eq(payments.billId, bill.id), eq(payments.status, "SUCCESS")),
        );
      ...
      const paymentTotals = await tx
        ...
        .where(
          and(
            eq(payments.orderId, billingOrderId),
      ...
      const totals = new Map<string, number>(
        paymentTotals
          .filter(
            (row): row is typeof row & { billId: string } =>
              row.billId !== null,
          )
      ...
      if (orderPaid && billingOrder.status === "BILL_REQUESTED") {
        const paidOrders = await tx
          ...
          .returning();
        const paid = paidOrders.find(
          (candidate) => candidate.id === billingOrderId,
        );
        if (paid) {
          updatedOrder = paid;
          await tx.insert(orderStatusHistory).values(
            combinedOrders
              .filter((candidate) => candidate.status === "BILL_REQUESTED")
              .map((candidate) => ({
                orderId: candidate.id,
                oldStatus: "BILL_REQUESTED" as const,
                newStatus: "PAID" as const,
                changedBy: data.changedBy,
                reason: "Combined payment completed",
              })),
          );
  ...
  async splitOrderEvenly(data: {
    ...
    return db.transaction(async (tx) => {
      ...
      const existingPayments = await tx.query.payments.findFirst({
      ...
      if (!itemIdsByBill)
        return {
          status: "too_many_bills" as const,
          orderBranchId: order.branchId,
        };
      const oldBills = await tx.query.bills.findMany({
        where: eq(bills.orderId, data.orderId),
      });
      if (oldBills.length) {
        await tx.delete(billOrderItems).where(
          inArray(
            billOrderItems.billId,
            oldBills.map((bill) => bill.id),
          ),
        );
      ...
      const discounts = splitMoneyEvenly(
        parseFloat(order.discountAmount),
        data.ways,
      );
      const serviceCharges = splitMoneyEvenly(
        parseFloat(order.serviceChargeAmount),
        data.ways,
      );
      const roundingAdjustments = splitMoneyEvenly(
        parseFloat(order.roundingAdjustment),
        data.ways,
      );
      const created = await tx
        ...
        .returning();
      await tx.insert(billOrderItems).values(
        itemIdsByBill.flatMap((itemIds, billIndex) =>
  ...
  async splitOrderByItems(data: {
    ...
    return db.transaction(async (tx) => {
      ...
      const validation = validateItemAllocations(
        activeItems.map((item) => item.id),
        data.allocations,
      );
      if (!validation.ok)
        return {
          status: "invalid_allocation" as const,
          reason: validation.reason,
          orderBranchId: order.branchId,
        };
      const comboValidation = validateComboGroupAllocations(
        activeItems,
        data.allocations,
      );
      ...
      const weights = data.allocations.map((allocation) =>
      ...
      const subtotals = data.allocations.map((allocation) =>
        allocation.orderItemIds.reduce(
          (sum, id) => sum + Number(itemsById.get(id)!.subtotal),
          0,
        ),
      );
      const taxes = allocateTotalsByWeight(Number(order.taxAmount), weights);
      const discounts = allocateTotalsByWeight(
        Number(order.discountAmount),
        weights,
      );
      const serviceCharges = allocateTotalsByWeight(
        Number(order.serviceChargeAmount),
        weights,
      );
      const roundingAdjustments = allocateTotalsByWeight(
      ...
      await tx.insert(billOrderItems).values(
        data.allocations.flatMap((allocation, index) =>
          allocation.orderItemIds.map((orderItemId) => ({
            billId: created[index]!.id,
            orderItemId,
          })),
        ),
      );
  ...
  async replaceSeatShares(data: {
    ...
    return db.transaction(async (tx) => {
      ...
      await tx
        .delete(orderItemSeatShares)
        .where(eq(orderItemSeatShares.orderItemId, data.orderItemId));
      if (data.shares.length) {
        await tx.insert(orderItemSeatShares).values(
          data.shares.map((share) => ({
            orderItemId: data.orderItemId,
            seatLabel: share.seatLabel.trim(),
            shareRatio: share.shareRatio.toFixed(6),
          })),
        );
      }
  ...
  async splitOrderByShares(data: {
    ...
    return db.transaction(async (tx) => {
      ...
      const validation = validateItemShareAllocations(
        activeItems.map((item) => item.id),
        data.allocations,
      );
      if (!validation.ok)
      ...
      const comboValidation = validateFractionalComboAllocations(
        activeItems,
        data.allocations,
      );
      if (!comboValidation.ok)
      ...
      await tx.insert(billOrderItems).values(
        data.allocations.flatMap((allocation, index) =>
          allocation.itemShares.map((share) => ({
            billId: created[index]!.id,
            orderItemId: share.orderItemId,
            allocationRatio: share.shareRatio.toFixed(6),
          })),
        ),
      );
      return {
  ...
  async findBillsByOrder(data: {
    ...
    const orderBills = await db.query.bills.findMany({
      ...
      with: {
        payments: true,
        itemAssignments: {
          with: { orderItem: { with: { order: { with: { table: true } } } } },
        },
      },
    });
  ...
  async findActiveItemsForSeatSplit(data: {
    orderId: string;
    tenantId: string;
    branchId: string | null;
  }) {
