...
describe('resolveRefundEligibility', () => {
  ...
  it('rejects non-successful payments', () => {
    expect(resolveRefundEligibility('PENDING', 100, 10)).toEqual({ ok: false, reason: 'PAYMENT_NOT_REFUNDABLE' });
  });
  it('rejects refunds larger than the payment', () => {
    expect(resolveRefundEligibility('SUCCESS', 100, 100.01)).toEqual({ ok: false, reason: 'REFUND_AMOUNT_EXCEEDS_PAYMENT' });
  });
});
