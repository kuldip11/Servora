...
import { billNotFound, orderNotFound, paymentNotFound, paymentNotRefundable, refundExceedsPaymentAmount } from '../billing.errors';
...
describe('billing errors', () => {
  ...
  it('maps refund rules to domain-rule errors', () => {
    expect(paymentNotRefundable().toJSON()).toMatchObject({ code: 'DOMAIN_RULE_VIOLATION' });
