...
const {getDashboard}=vi.hoisted(()=>({getDashboard:vi.fn().mockResolvedValue({ success:true, data:{} })}));
vi.mock('../analytics.controller', () => ({ analyticsController:{ getDashboard } }));
import { analyticsRouter } from '../analytics.route';

describe('analytics routes', () => {
  it('registers the protected dashboard endpoint', () => {
    expect((analyticsRouter as any).routes).toEqual(expect.arrayContaining([
      expect.objectContaining({ method:'GET', path:'/api/analytics/dashboard' }),
    ]));
  });
});
