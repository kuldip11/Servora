...
export const tenantService = {
  async list(auth: AuthContext) {
    ...
    const memberships = await tenantRepository.findMembershipsByUserId(auth.userId);
  ...
  async create(auth: AuthContext, input: { name: string }) {
    ...
    const tenant = await tenantRepository.create({ name: input.name, createdBy: auth.userId });
    const tenantRole = await tenantRepository.findRoleByName('FRANCHISE_ADMIN');
    if (!tenantRole || tenantRole.scope !== 'TENANT') throw new Error('RBAC reference data is not installed: TENANT FRANCHISE_ADMIN role is missing');
    const membership = await tenantRepository.createOwnerMembership(auth.userId, tenant.id, tenantRole.id);
    await writeAudit({ tenantId: tenant.id, userId: auth.userId, action: 'TENANT_CREATED', entity: 'tenant', entityId: tenant.id, metadata: { name: tenant.name } });
  ...
  async update(auth: AuthContext, tenantId: string, changes: { name?: string }) {
    ...
    if (!auth.roles.includes('OWNER') && tenantId !== auth.tenantId) throw tenantNotFound(tenantId);
    const updated = await tenantRepository.update(tenantId, changes);
    if (!updated) throw tenantNotFound(tenantId);
    await writeAudit({ tenantId, userId: auth.userId, action: 'TENANT_UPDATED', entity: 'tenant', entityId: tenantId, metadata: { changes } });
    return updated;
  },
  ...
  async archive(auth: AuthContext, tenantId: string) {
    ...
    if (!auth.roles.includes('OWNER') && tenantId !== auth.tenantId) throw tenantNotFound(tenantId);
    const updated = await tenantRepository.update(tenantId, { isActive: false });
    if (!updated) throw tenantNotFound(tenantId);
    await writeAudit({ tenantId, userId: auth.userId, action: 'TENANT_ARCHIVED', entity: 'tenant', entityId: tenantId });
    return updated;
  },
};
