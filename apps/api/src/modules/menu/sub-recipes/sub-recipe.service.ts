...
import {
  subRecipeRepository,
  type SubRecipeIngredientWrite,
} from "./sub-recipe.repository";
...
const validateSources = async (
  ...
  if (!ingredients.length) {
    throw new ValidationError(
      "A sub-recipe must contain at least one ingredient",
    );
  }
  ...
  for (const ingredient of ingredients) {
    if (
      Number(Boolean(ingredient.inventoryItemId)) +
        Number(Boolean(ingredient.ingredientSubRecipeId)) !==
      1
    ) {
      throw new ValidationError(
        "Each sub-recipe ingredient must reference exactly one raw inventory item or sub-recipe",
      );
    }
    if (ingredient.quantity <= 0)
      throw new ValidationError(
        "Sub-recipe ingredient quantity must be greater than zero",
      );
    if (ingredient.inventoryItemId) rawIds.push(ingredient.inventoryItemId);
    if (ingredient.ingredientSubRecipeId)
      nestedIds.push(ingredient.ingredientSubRecipeId);
  }

  const [rawSources, nestedSources] = await Promise.all([
    subRecipeRepository.findOwnedInventorySources(tenantId, branchId, rawIds),
    subRecipeRepository.findOwnedSubRecipeSources(
      tenantId,
      branchId,
      nestedIds,
    ),
  ...
  const nestedById = new Map(
    nestedSources.map((row) => [row.id, row] as const),
  );
  ...
  for (const ingredient of ingredients) {
    if (ingredient.inventoryItemId) {
      ...
      if (!source) {
        throw new ValidationError(
          "Sub-recipe references inventory outside the active branch",
        );
      }
      ...
      if (!source) {
        throw new ValidationError(
          "Sub-recipe references a component outside the active branch",
        );
      }
...
const validateYield = (input: SubRecipeInput) => {
  if (!input.name.trim())
    throw new ValidationError("Sub-recipe name is required");
  if (input.yieldQuantity <= 0)
    throw new ValidationError(
      "Sub-recipe yield quantity must be greater than zero",
    );
  if (
    input.yieldPercent != null &&
    (input.yieldPercent <= 0 || input.yieldPercent > 100)
  ) {
    throw new ValidationError(
...
export const subRecipeService = {
  ...
  async create(auth: AuthContext, input: SubRecipeInput) {
    ...
    const branchId = requireBranch(
      auth,
      "Select a branch before creating a sub-recipe",
    );
    validateYield(input);
    await validateSources(auth.tenantId, branchId, input.ingredients);
    await assertGraphSafe(
      auth.tenantId,
      null,
      input.ingredients.flatMap((i) =>
        i.ingredientSubRecipeId ? [i.ingredientSubRecipeId] : [],
      ),
    );
    const created = await subRecipeRepository.create({
  ...
  async update(auth: AuthContext, id: string, input: SubRecipeInput) {
    ...
    const branchId = requireBranch(
      auth,
      "Select a branch before editing a sub-recipe",
    );
    const existing = await subRecipeRepository.findById(auth.tenantId, id);
    if (!existing || existing.branchId !== branchId)
      throw new NotFoundError("Sub-recipe");
    validateYield(input);
    if (
      input.ingredients.some(
        (ingredient) => ingredient.ingredientSubRecipeId === id,
      )
    ) {
    ...
    const updated = await subRecipeRepository.update(id, {
      branchId,
      ...input,
    });
  ...
  async delete(auth: AuthContext, id: string) {
    ...
    const branchId = requireBranch(
      auth,
      "Select a branch before deleting a sub-recipe",
    );
    const existing = await subRecipeRepository.findById(auth.tenantId, id);
    if (!existing || existing.branchId !== branchId)
      throw new NotFoundError("Sub-recipe");
    ...
    for (const ref of references) {
      const entry = byItem.get(ref.menuItemId) ?? {
        variants: [],
        modifiers: [],
      };
