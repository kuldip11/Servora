...
export const subRecipesRouter = new Elysia({ prefix: "/api/menu/sub-recipes" })
  ...
  .get("/", async ({ auth }) =>
    successResponse(await subRecipeService.list(auth)),
  )
  .post(
    "/",
    async ({ auth, body }) =>
      createdResponse(await subRecipeService.create(auth, body)),
    { body: subRecipeBody },
  )
  .put(
    "/:id",
    async ({ auth, params, body }) =>
      successResponse(await subRecipeService.update(auth, params.id, body)),
    { params: subRecipeParams, body: subRecipeBody },
  )
  .delete(
    "/:id",
    async ({ auth, params }) => {
