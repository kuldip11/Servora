...
export type MenuChangeEntityType =
  | "MENU_ITEM" | "VARIANT" | "MODIFIER_GROUP" | "MODIFIER_OPTION"
  | "CATEGORY" | "MENU" | "MENU_MEMBERSHIP" | "PRICE_RULE" | "PROMOTION"
  | "RECIPE" | "SUB_RECIPE" | "TEMPLATE" | "AVAILABILITY" | "TAG";
export type MenuChangeType = "CREATED" | "UPDATED" | "PUBLISHED" | "ARCHIVED" | "DELETED";

export type ChangeDiff = Record<string, { old: unknown; new: unknown } | unknown>;
...
export const menuChangeLog = {
  async record(
    ...
    const [event] = await db.insert(menuChangeEvents).values({
  ...
  async recordMany(
    ...
    return db.insert(menuChangeEvents).values(events.map((event) => ({
      ...event,
  ...
  async list(tenantId: string, filters: {
    ...
    return db.query.menuChangeEvents.findMany({
      where: and(
        ...
        filters.entityType ? eq(menuChangeEvents.entityType, filters.entityType) : undefined,
        filters.entityId ? eq(menuChangeEvents.entityId, filters.entityId) : undefined,
        filters.changeType ? eq(menuChangeEvents.changeType, filters.changeType) : undefined,
        filters.before ? lt(menuChangeEvents.changedAt, filters.before) : undefined,
      ),
  ...
  async latestForItems(tenantId: string, itemIds: string[], asOf: Date) {
    ...
    const rows = await db.select({ id: menuChangeEvents.id, entityId: menuChangeEvents.entityId })
    ...
    for (const row of rows) if (!latest.has(row.entityId)) latest.set(row.entityId, row.id);
    return latest;
  },
};
