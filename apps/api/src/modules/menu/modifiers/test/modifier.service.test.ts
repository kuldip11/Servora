...
import { modifierService } from "@/modules/menu/modifiers/modifier.service";
...
describe("modifierService conditional dependency validation", () => {
  it("rejects an arbitrary-depth circular dependency at write time", async () => {
    findModifierGroup.mockImplementation(
      async (_tenantId: string, groupId: string) => {
        if (groupId === "group-a")
          return {
            id: "group-a",
            branchId: "branch-1",
        ...
        if (groupId === "group-c")
          return {
            id: "group-c",
            branchId: "branch-1",
            dependsOnOptionId: "option-a",
            groupType: "ADDON",
          };
        return undefined;
      },
    );
    findModifierOption.mockImplementation(
      async (_tenantId: string, optionId: string) => {
        if (optionId === "option-b")
          return { id: optionId, modifierGroupId: "group-b" };
        if (optionId === "option-c")
          return { id: optionId, modifierGroupId: "group-c" };
        if (optionId === "option-a")
          return { id: optionId, modifierGroupId: "group-a" };
        return undefined;
      },
    );
