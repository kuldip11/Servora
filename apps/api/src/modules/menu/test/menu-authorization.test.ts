...
import { assertMenuQueryBranch, assertMenuResourceBranch, resolveMenuBranch } from '../menu-authorization';

const tenantWide:any={tenantWide:true,branchId:null,authorizedBranchIds:['b1']};
const branchScoped:any={tenantWide:false,branchId:'b1',authorizedBranchIds:['b1']};
describe('menu authorization',()=>{
  ...
  it('allows shared resources only when explicitly enabled',()=>{
    ...
    expect(()=>assertMenuResourceBranch(branchScoped,null,{allowShared:true})).not.toThrow();
  });
});
