...
import { createItemBody, updateItemBody, duplicateItemBody, updateItemStatusBody, updateItemAvailabilityBody, itemIdParams, itemStatusParams, itemStatusQuery } from '../item.validator';
...
describe('item.validator validators', () => {
  it('requires category, name, and base price for item creation', () => {
    ...
    expect(Value.Check(createItemBody, { categoryId: 'c1', name: 'Tea', basePrice: 10 })).toBe(true);
  ...
  it('validates status, availability, and id params', () => {
    ...
    expect(Value.Check(updateItemAvailabilityBody, { isAvailable: true })).toBe(true);
