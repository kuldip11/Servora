...
const validate = (input: PromotionInput) => {
  if (input.ruleType === "PERCENTAGE") {
    if (input.value === undefined || input.value <= 0 || input.value > 100) {
      throw new ValidationError(
        "Percentage promotion must be greater than 0 and at most 100%",
      );
    }
  } else if (input.ruleType === "FIXED_AMOUNT") {
    if (input.value === undefined || input.value <= 0)
      throw new ValidationError(
        "Fixed promotion amount must be greater than 0",
      );
  } else {
    const triggerTargets =
      Number(Boolean(input.triggerMenuItemId)) +
      Number(Boolean(input.triggerCategoryId));
    const rewardTargets =
      Number(Boolean(input.rewardMenuItemId)) +
      Number(Boolean(input.rewardCategoryId));
    if (triggerTargets !== 1)
      throw new ValidationError(
        "BOGO requires exactly one trigger item or category",
      );
    if (rewardTargets > 1)
      throw new ValidationError(
        "BOGO reward can target at most one item or category",
      );
    if (
      !input.rewardDiscountPercent ||
      input.rewardDiscountPercent <= 0 ||
      input.rewardDiscountPercent > 100
    ) {
      throw new ValidationError(
        "BOGO reward discount must be greater than 0 and at most 100%",
      );
    }
    if (
      !input.triggerQuantity ||
      input.triggerQuantity < 1 ||
      !input.rewardQuantity ||
      input.rewardQuantity < 1
    ) {
      throw new ValidationError(
        "BOGO trigger and reward quantities must be positive integers",
      );
    }
  }
  if (input.ruleType === "BOGO") {
    if (
      input.scope !== "ORDER" ||
      input.scopeCategoryId ||
      input.scopeMenuItemId
    ) {
      throw new ValidationError(
        "BOGO scope is defined by its buy/reward targets and must use ORDER scope",
      );
    }
  } else {
    if (
      input.scope === "ORDER" &&
      (input.scopeCategoryId || input.scopeMenuItemId)
    )
      throw new ValidationError(
        "Order promotions cannot target an item/category",
      );
    if (
      ...
      throw new ValidationError(
        "Category promotion requires exactly one category target",
      );
    if (
      input.scope === "ITEM" &&
      (!input.scopeMenuItemId || input.scopeCategoryId)
    )
      throw new ValidationError(
        "Item promotion requires exactly one item target",
      );
  }
  if (input.startDate && input.endDate && input.startDate > input.endDate)
    throw new ValidationError("Promotion start date must be before end date");
};

const validateTargets = async (auth: AuthContext, input: PromotionInput) => {
  const itemIds = new Set<string>();
  const categoryIds = new Set<string>();
  for (const id of [
    input.scopeMenuItemId,
    input.triggerMenuItemId,
    input.rewardMenuItemId,
  ]) {
    if (id) itemIds.add(id);
  }
  for (const id of [
    input.scopeCategoryId,
    input.triggerCategoryId,
    input.rewardCategoryId,
  ]) {
  ...
  for (const itemId of itemIds) {
    ...
    if (!item)
      throw new ValidationError(
        "Promotion menu item does not belong to this tenant",
      );
  ...
  for (const categoryId of categoryIds) {
    ...
    if (!category)
      throw new ValidationError(
        "Promotion category does not belong to this tenant",
      );
    assertMenuResourceBranch(auth, category.branchId, { allowShared: true });
  }
};

const persistence = (input: PromotionInput): NewPromotion => {
  validate(input);
  return {
    ...
    triggerMenuItemId:
      input.ruleType === "BOGO" ? (input.triggerMenuItemId ?? null) : null,
    triggerCategoryId:
      input.ruleType === "BOGO" ? (input.triggerCategoryId ?? null) : null,
    rewardMenuItemId:
      input.ruleType === "BOGO" ? (input.rewardMenuItemId ?? null) : null,
    rewardCategoryId:
      input.ruleType === "BOGO" ? (input.rewardCategoryId ?? null) : null,
    rewardDiscountPercent:
      input.ruleType === "BOGO"
        ? input.rewardDiscountPercent!.toFixed(2)
        : null,
...
export const promotionService = {
  async preview(auth: AuthContext, input: PromotionPreviewInput) {
    ...
    if (!input.items.length)
      throw new ValidationError(
        "Add at least one menu item to preview this promotion",
      );
    ...
    const merged: PromotionInput = {
      ...
      ...(patch.value !== undefined
        ...
        : existing.value !== null
          ...
          : {}),
      ...(patch.scopeCategoryId !== undefined
        ? { scopeCategoryId: patch.scopeCategoryId }
        : existing.scopeCategoryId
          ? { scopeCategoryId: existing.scopeCategoryId }
          : {}),
      ...(patch.scopeMenuItemId !== undefined
        ? { scopeMenuItemId: patch.scopeMenuItemId }
        : existing.scopeMenuItemId
          ? { scopeMenuItemId: existing.scopeMenuItemId }
          : {}),
      ...
      ...(patch.endTime !== undefined
        ...
        : existing.endTime
          ...
          : {}),
      maxUsesTotal:
        patch.maxUsesTotal !== undefined
          ? patch.maxUsesTotal
          : existing.maxUsesTotal,
      maxUsesPerCustomer:
        patch.maxUsesPerCustomer !== undefined
          ? patch.maxUsesPerCustomer
          : existing.maxUsesPerCustomer,
      ...(patch.triggerMenuItemId !== undefined
        ? { triggerMenuItemId: patch.triggerMenuItemId }
        : existing.triggerMenuItemId
          ? { triggerMenuItemId: existing.triggerMenuItemId }
          : {}),
      ...(patch.triggerCategoryId !== undefined
        ? { triggerCategoryId: patch.triggerCategoryId }
        : existing.triggerCategoryId
          ? { triggerCategoryId: existing.triggerCategoryId }
          : {}),
      ...(patch.rewardMenuItemId !== undefined
        ? { rewardMenuItemId: patch.rewardMenuItemId }
        : existing.rewardMenuItemId
          ? { rewardMenuItemId: existing.rewardMenuItemId }
          : {}),
      ...(patch.rewardCategoryId !== undefined
        ? { rewardCategoryId: patch.rewardCategoryId }
        : existing.rewardCategoryId
          ? { rewardCategoryId: existing.rewardCategoryId }
          : {}),
      ...(patch.rewardDiscountPercent !== undefined
        ? { rewardDiscountPercent: patch.rewardDiscountPercent }
        : existing.rewardDiscountPercent
          ? { rewardDiscountPercent: Number(existing.rewardDiscountPercent) }
          : {}),
      ...(patch.triggerQuantity !== undefined
        ? { triggerQuantity: patch.triggerQuantity }
        : existing.triggerQuantity
          ? { triggerQuantity: existing.triggerQuantity }
          : {}),
      ...(patch.rewardQuantity !== undefined
        ? { rewardQuantity: patch.rewardQuantity }
        : existing.rewardQuantity
          ? { rewardQuantity: existing.rewardQuantity }
          : {}),
      stackableWithLoyalty:
        patch.stackableWithLoyalty ?? existing.stackableWithLoyalty,
