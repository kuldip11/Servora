...
export const promotionController = {
  async preview(auth: AuthContext, input: PromotionPreviewInput) {
    return successResponse(await promotionService.preview(auth, input));
  },
  async list(auth: AuthContext) {
    return successResponse(await promotionService.list(auth));
  },
  async create(auth: AuthContext, input: PromotionInput) {
    return createdResponse(await promotionService.create(auth, input));
  },
  async update(auth: AuthContext, id: string, input: Partial<PromotionInput>) {
    return successResponse(await promotionService.update(auth, id, input));
  },
