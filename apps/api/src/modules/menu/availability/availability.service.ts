...
type ScheduleRow = Awaited<ReturnType<typeof availabilityRepository.findActiveSchedulesForItem>>[number];
...
export const availabilityService = {
  ...
  async createSchedule(tenantId: string, itemId: string, input: CreateScheduleInput) {
    const item = await availabilityRepository.findItemBasics(tenantId, itemId);
    if (!item) throw itemNotFound(itemId);

    if (input.scheduleType === 'DAILY' && (!input.startTime || !input.endTime)) {
      throw invalidScheduleFields('DAILY schedules need startTime and endTime');
    }
    if (input.scheduleType === 'WEEKLY' && (input.dayOfWeek == null || !input.startTime || !input.endTime)) {
      throw invalidScheduleFields('WEEKLY schedules need dayOfWeek, startTime and endTime');
    }
    if (input.scheduleType === 'SPECIFIC_DATE' && !input.startDate) {
      throw invalidScheduleFields('SPECIFIC_DATE schedules need at least a startDate');
    }
    if (input.scheduleType === 'HOLIDAY' && !input.holidayName) {
      throw invalidScheduleFields('HOLIDAY schedules need a holidayName');
    }

    return availabilityRepository.createSchedule({ tenantId, menuItemId: itemId, ...input });
  },

  async updateSchedule(tenantId: string, scheduleId: string, input: UpdateScheduleInput) {
    const updated = await availabilityRepository.updateSchedule(tenantId, scheduleId, input);
  ...
  async isScheduleActive(tenantId: string, schedule: ScheduleRow, now: Date): Promise<boolean> {
    if (!schedule.isActive) return false;
    switch (schedule.scheduleType) {
      case 'DAILY':
        if (!schedule.startTime || !schedule.endTime) return false;
        return timeInRange(formatTime(now), schedule.startTime, schedule.endTime);

      case 'WEEKLY':
        if (schedule.dayOfWeek == null || !schedule.startTime || !schedule.endTime) return false;
        ...
        // special.
        return now.getDay() === schedule.dayOfWeek && timeInRange(formatTime(now), schedule.startTime, schedule.endTime);
      ...
      case 'HOLIDAY': {
        ...
        const today = formatDate(now);
        const match = await availabilityRepository.findHoliday(tenantId, schedule.holidayName, today);
        return !!match;
      }
  ...
  async getEffectiveStatus(
    ...
    const schedules = await availabilityRepository.findActiveSchedulesForItem(tenantId, itemId, branchId);

    let best: { schedule: ScheduleRow; priority: number } | null = null;
    for (const schedule of schedules) {
      const active = await availabilityService.isScheduleActive(tenantId, schedule, at);
      if (!active) continue;
    ...
    if (best) {
      return { status: best.schedule.statusDuringPeriod, reason: describeSchedule(best.schedule) };
    }
    return { status: item.status, reason: 'No active schedule — using base status' };
  },
  ...
  async getItemsAvailableAt(tenantId: string, branchId: string, at: Date = new Date()) {
    const items = await availabilityRepository.listActiveItemBasics(tenantId);
    const available: string[] = [];
    for (const item of items) {
      const { status } = await availabilityService.getEffectiveStatus(tenantId, item.id, branchId, at);
  ...
  async upsertOverride(tenantId: string, itemId: string, branchId: string, input: UpsertOverrideInput) {
    const item = await availabilityRepository.findItemBasics(tenantId, itemId);
    if (!item) throw itemNotFound(itemId);
  ...
  async deleteOverride(tenantId: string, itemId: string, branchId: string): Promise<void> {
  ...
  async getEffectiveItem(tenantId: string, itemId: string, branchId: string) {
    ...
    const effectivePrepTimeMinutes = override?.prepTimeMinutes ?? item.prepTimeMinutes;
    ...
    return {
      ...
      availabilityReason: override?.availabilityReason ?? item.availabilityReason,
