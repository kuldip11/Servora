...
describe("shared schedule precedence", () => {
  it("uses inclusive daypart boundaries", async () => {
    expect(
      await scheduleMatches(daily, new Date(2026, 7, 29, 6, 0), noHoliday),
    ).toBe(true);
    expect(
      await scheduleMatches(daily, new Date(2026, 7, 29, 11, 0), noHoliday),
    ).toBe(true);
    expect(
      await scheduleMatches(daily, new Date(2026, 7, 29, 11, 0, 1), noHoliday),
    ).toBe(false);
  });
  it("chooses holiday over an overlapping weekly window", async () => {
    ...
    expect(
      await highestPriorityActiveSchedule(
        [weekly, holiday],
        new Date(2026, 7, 29, 8),
        async () => true,
      ),
    ).toBe(holiday);
  });
