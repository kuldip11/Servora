...
import { createScheduleBody, updateScheduleBody, itemIdParams, scheduleIdParams, itemBranchParams, currentStatusQuery, holidayQuery, createHolidayBody, updateHolidayBody, holidayIdParams, upsertOverrideBody } from '../availability.validator';
...
describe('availability.validator validators', () => {
  it('requires scheduleType for schedule creation', () => {
    ...
    expect(Value.Check(createScheduleBody, { scheduleType: 'DAILY' })).toBe(true);
  ...
  it('validates required params and allows optional query/body fields', () => {
    ...
    expect(Value.Check(itemBranchParams, { id: 'i1', branchId: 'b1' })).toBe(true);
