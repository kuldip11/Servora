...
const permissionFor = (pathname: string, method: string): MenuPermission => {
  if (/\/manual-override$/.test(pathname)) return "menu:update";
  if (/\/publish$|\/unpublish$/.test(pathname)) return "menu:publish";
  if (pathname.startsWith("/api/menu/price-rules") && method !== "GET")
    return "menu:pricing:write";
...
export const menuAuthorizationPlugin = () =>
  new Elysia({ name: "menu-authorization" })
    ...
    .onBeforeHandle(({ request, auth }) => {
      ...
      requireMenuPermission(auth, permissionFor(pathname, request.method));
    });
