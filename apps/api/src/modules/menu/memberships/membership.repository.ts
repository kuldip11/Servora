...
export const membershipRepository = {
  async listForItem(tenantId: string, menuItemId: string) {
    return db.query.menuMemberships.findMany({
      where: eq(menuMemberships.menuItemId, menuItemId),
      with: { menu: true, category: true, item: { columns: { tenantId: true } } },
  ...
  async findResources(tenantId: string, menuId: string, itemId: string, categoryId: string) {
    const [menu, item, category] = await Promise.all([
      db.query.menus.findFirst({ where: and(eq(menus.id, menuId), eq(menus.tenantId, tenantId)) }),
      db.query.menuItems.findFirst({ where: and(eq(menuItems.id, itemId), eq(menuItems.tenantId, tenantId)) }),
      db.query.menuCategories.findFirst({ where: and(eq(menuCategories.id, categoryId), eq(menuCategories.tenantId, tenantId)) }),
    ]);
    return { menu, item, category };
  },

  async upsert(data: { menuId: string; menuItemId: string; categoryId: string; sortOrder: number }) {
    const [membership] = await db
      .insert(menuMemberships)
      .values(data)
      .onConflictDoUpdate({
        target: [menuMemberships.menuId, menuMemberships.menuItemId],
        set: { categoryId: data.categoryId, sortOrder: data.sortOrder, updatedAt: new Date() },
      })
      .returning();
    return membership!;
  },
  ...
  async remove(tenantId: string, menuItemId: string, menuId: string) {
    const [item, menu] = await Promise.all([
      db.query.menuItems.findFirst({
        where: and(eq(menuItems.id, menuItemId), eq(menuItems.tenantId, tenantId)),
        columns: { id: true },
      }),
    ...
    const rows = await db
      ...
      .where(and(eq(menuMemberships.menuItemId, menuItemId), eq(menuMemberships.menuId, menuId)))
