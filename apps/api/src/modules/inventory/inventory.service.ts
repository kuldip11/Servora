...
import { requireInventoryPermission, requireInventoryTransactionPermission, resolveInventoryBranch, assertInventoryResourceBranch } from './inventory-authorization';
...
export const inventoryService = {
  ...
  async list(auth: AuthContext) {
    ...
    if (auth.tenantWide && !auth.branchId) return inventoryRepository.findAllBranches(auth.tenantId);
  ...
  async create(auth: AuthContext, input: CreateInventoryItemInput) {
    ...
    if (parseFloat(item.currentStock) <= parseFloat(item.minimumStock)) {
      await eventBus.publish({ type: 'inventory.low_stock', payload: item as any }, auth.tenantId, branchId);
    }
  ...
  async updateStock(auth: AuthContext, itemId: string, input: UpdateStockInput) {
    ...
    if (currentStock <= minimumStock) {
      await eventBus.publish({ type: 'inventory.low_stock', payload: result.item as any }, auth.tenantId, branchId);
    }
    ...
    await inventoryService.syncMenuItemAvailability(auth.tenantId, branchId, [itemId]);
  ...
  async lowStockAlerts(auth: AuthContext) {
    requireInventoryPermission(auth, 'inventory:read');
    if (auth.tenantWide && !auth.branchId) return inventoryRepository.findLowStockAllBranches(auth.tenantId);
    const branchId = resolveInventoryBranch(auth);
  ...
  async validateStock(
  ...
  ): Promise<{ valid: boolean; insufficient: Array<{ menuItemId: string; inventoryItemId: string; name: string }> }> {
    const menuItemIds = items.map((i) => i.menuItemId);
    if (!menuItemIds.length) return { valid: true, insufficient: [] };

    const recipeLines = await inventoryRepository.findRequiredRecipeLines(menuItemIds);

    const requiredByInventoryItem = new Map<string, number>();
    for (const item of items) {
      for (const r of recipeLines) {
        if (r.menuItemId !== item.menuItemId || !r.menuItem.enableRecipeDeduction) continue;
        const need = parseFloat(r.quantityRequired) * item.quantity;
        requiredByInventoryItem.set(r.inventoryItemId, (requiredByInventoryItem.get(r.inventoryItemId) ?? 0) + need);
      }
    }
    ...
    for (const r of recipeLines) {
      ...
      const available = stockCache.get(r.inventoryItemId) ?? parseFloat(r.inventoryItem.currentStock);
      stockCache.set(r.inventoryItemId, available);
      const totalNeeded = requiredByInventoryItem.get(r.inventoryItemId)!;
      if (totalNeeded > available && !insufficient.some((x) => x.inventoryItemId === r.inventoryItemId)) {
        insufficient.push({ menuItemId: r.menuItemId, inventoryItemId: r.inventoryItemId, name: r.inventoryItem.name });
      }
    }
  ...
  async deductForOrderItems(
    ...
    const lines: Array<{ inventoryItemId: string; menuItemId: string; unit: InventoryUnit; neededQuantity: number }> = [];
    for (const item of items) {
      const itemRecipes = recipeLines.filter((r) => r.menuItemId === item.menuItemId && r.menuItem.enableRecipeDeduction);
    ...
    if (touchedInventoryItemIds.length) {
      ...
      for (const item of touchedItems) {
        if (parseFloat(item.currentStock) <= parseFloat(item.minimumStock)) {
          await eventBus.publish({ type: 'inventory.low_stock', payload: item as any }, tenantId, branchId);
        }
      }
    }
  ...
  async syncMenuItemAvailability(tenantId: string, branchId: string, inventoryItemIds: string[]) {
    if (!inventoryItemIds.length) return;

    const affectedItemIds = await inventoryRepository.findAffectedMenuItemIds(inventoryItemIds, branchId);
    if (!affectedItemIds.length) return;

    const items = await inventoryRepository.findMenuItemsForAvailability(tenantId, branchId, affectedItemIds);
    ...
    for (const item of items) {
      ...
      const ingredients = await inventoryRepository.findNonOptionalIngredients(tenantId, branchId, item.id);
      const canOrder = ingredients.every(
        (ing) => parseFloat(ing.inventoryItem.currentStock) >= parseFloat(ing.quantityRequired),
      );

      if (!canOrder && item.status === 'ACTIVE') {
        await inventoryRepository.setMenuItemAvailabilityStatus(item.id, 'OUT_OF_STOCK', 'Insufficient inventory');
      } else if (canOrder && item.status === 'OUT_OF_STOCK') {
        await inventoryRepository.setMenuItemAvailabilityStatus(item.id, 'ACTIVE', null);
      }
    }
  },
};
