...
vi.mock('../inventory.controller', () => ({ inventoryController:{ list, create, updateStock, lowStockAlerts } }));
import { inventoryRouter } from '../inventory.route';
describe('inventory routes',()=>{ it('registers item, stock, and low-stock endpoints',()=>{ expect((inventoryRouter as any).routes).toEqual(expect.arrayContaining([
  expect.objectContaining({method:'GET',path:'/api/inventory/items'}),
  expect.objectContaining({method:'POST',path:'/api/inventory/items'}),
  expect.objectContaining({method:'PATCH',path:'/api/inventory/items/:id/stock'}),
  expect.objectContaining({method:'GET',path:'/api/inventory/alerts/low-stock'}),
])); }); });
