...
describe("inventory void reversal transaction contract", () => {
  const source = readFileSync(new URL("../../orders/order.repository.ts", import.meta.url), "utf8");
  const method = source.slice(source.indexOf("async voidItem("), source.indexOf("async compItem("));
  ...
  it("E5 reverses every persisted deduction row for the voided order item without re-resolving recipes", () => {
    expect(method).toContain("inArray(orderInventoryDeductions.orderItemId, activeIds)");
