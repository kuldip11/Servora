...
export function requireInventoryPermission(auth: AuthContext, permission: InventoryPermission): void {
  requirePermission(auth, permission);
}

/** Resolve a concrete inventory branch while enforcing membership scope. */
export function resolveInventoryBranch(auth: AuthContext, requestedBranchId?: string | null): string {
  const requested = requestedBranchId ?? auth.branchId ?? undefined;
  if (!requested) return requireBranch(auth, 'Please select a specific branch before accessing inventory.');

  if (auth.tenantWide) return requested;

  const active = requireBranch(auth, 'Please select a specific branch before accessing inventory.');
...
export function assertInventoryResourceBranch(auth: AuthContext, resourceBranchId: string | null | undefined): void {
