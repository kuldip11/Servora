...
const customerRequestType = t.Union([
  ...
  t.Literal("CUTLERY"),
  t.Literal("BILL"),
  t.Literal("ASSISTANCE"),
]);
const customerRequestStatus = t.Union([
...
export const customerRequestRouter = new Elysia()
  ...
  .get("/api/customer/requests", async ({ auth }) =>
    successResponse(await customerRequestService.listForStaff(auth)),
  )
  .patch(
      ...
      ),
    {
      ...
      body: t.Object({ status: customerRequestStatus }),
    },
  );
