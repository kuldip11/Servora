...
vi.mock('../branch.controller', () => ({ branchController: { list: vi.fn(), create: vi.fn(), update: vi.fn(), deactivate: vi.fn() } }));
import { branchesRouter } from '../branch.route';
describe('branch routes', () => { it('registers authenticated CRUD endpoints under /api/branches', () => { const routes = (branchesRouter as any).routes; expect(routes).toEqual(expect.arrayContaining([ expect.objectContaining({ method: 'GET', path: '/api/branches/' }), expect.objectContaining({ method: 'POST', path: '/api/branches/' }), expect.objectContaining({ method: 'PATCH', path: '/api/branches/:id' }), expect.objectContaining({ method: 'DELETE', path: '/api/branches/:id' }) ])); }); });
