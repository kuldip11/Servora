...
export const branchService = {
  ...
  async create(auth: AuthContext, input: CreateBranchInput) {
    ...
    const branch = await branchRepository.create({ tenantId: auth.tenantId, ...input });
    await writeAudit({ tenantId: auth.tenantId, userId: auth.userId, action: 'BRANCH_CREATED', entity: 'branch', entityId: branch.id, metadata: { name: branch.name } });
    return branch;
  },
  ...
  async update(auth: AuthContext, branchId: string, changes: UpdateBranchInput) {
    ...
    const touchesCapabilities = CAPABILITY_FIELDS.some((field) => changes[field] !== undefined);
    ...
    if (touchesCapabilities) {
      ...
      if (!merged.dineInEnabled && !merged.takeawayEnabled && !merged.deliveryEnabled && !merged.onlineEnabled) {
    ...
    const updated = await branchRepository.update(auth.tenantId, branchId, changes);
    if (!updated) throw branchNotFound(branchId);
    await writeAudit({ tenantId: auth.tenantId, userId: auth.userId, action: 'BRANCH_UPDATED', entity: 'branch', entityId: branchId, metadata: { changes } });
    return updated;
  },
  ...
  async deactivate(auth: AuthContext, branchId: string) {
    ...
    const hasOpenOrders = await branchRepository.hasOpenOrders(auth.tenantId, branchId);
    if (hasOpenOrders) throw branchHasOpenOrders();

    const deactivated = await branchRepository.update(auth.tenantId, branchId, { isActive: false });
    if (!deactivated) throw branchNotFound(branchId);
    await writeAudit({ tenantId: auth.tenantId, userId: auth.userId, action: 'BRANCH_ARCHIVED', entity: 'branch', entityId: branchId });
    return deactivated;
  },
};
