...
export const organizationController = {
  ...
  async update(auth: AuthContext, organizationId: string, changes: { name?: string }) {
    return successResponse(await organizationService.update(auth, organizationId, changes));
  },
  async archive(auth: AuthContext, organizationId: string) {
    return successResponse(await organizationService.archive(auth, organizationId));
  },
};
