...
export const customerGroupsRouter = new Elysia({
  ...
  .get(
    "/:id",
    ({ auth, params }) =>
      customerGroupService.findById(auth, params.id).then(successResponse),
    { params: customerGroupIdParams },
  )
  .post(
    "/",
    ({ auth, body }) =>
      customerGroupService.create(auth, body).then(createdResponse),
    { body: createCustomerGroupBody },
  )
  .patch(
    "/:id",
    ({ auth, params, body }) =>
      customerGroupService.update(auth, params.id, body).then(successResponse),
    { params: customerGroupIdParams, body: updateCustomerGroupBody },
  )
  .delete(
    "/:id",
    async ({ auth, params }) => {
