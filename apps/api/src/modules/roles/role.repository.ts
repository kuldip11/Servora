...
export const roleRepository = {
  ...
  findByNameAndScope(tenantId: string, name: string, scope: "TENANT" | "BRANCH") {
    return db.query.roles.findFirst({
      where: and(
  ...
  async create(tenantId: string, input: { name: string; description?: string; scope: "TENANT" | "BRANCH" }) {
  ...
  async assignmentCount(roleId: string) {
    const rows = await db.select({ count: sql<number>`count(*)::int` }).from(membershipRoles).where(eq(membershipRoles.roleId, roleId));
    return rows[0]?.count ?? 0;
  },

  async archive(id: string) {
    const [updated] = await db.update(roles).set({ isActive: false, updatedAt: new Date() }).where(eq(roles.id, id)).returning();
    return updated;
  },
};
