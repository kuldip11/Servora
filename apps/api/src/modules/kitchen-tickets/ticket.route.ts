...
export const kitchenTicketsRouter = new Elysia()
  ...
  .get("/api/kitchen-tickets/", ({ auth, query }) => ticketController.getQueue(auth, query.stationId), {
    query: t.Object({ stationId: t.Optional(t.String({ format: "uuid" })) }),
  })
  .get("/api/kitchen-tickets/stations", ({ auth }) => ticketController.listStations(auth))
  .patch(
