...
export const courseFireListener = {
  async onPriorCourseServed(auth: AuthContext, orderId: string) {
    const held = await ticketRepository.findAutoFireableHeldTickets(
      auth.tenantId,
      orderId,
    );
    ...
    for (const ticket of held) {
      const fired = await ticketRepository.setStatus(
        auth.tenantId,
        ticket.id,
        "FIRED",
        timestampFieldsFor("FIRED"),
      );
      if (!fired) continue;
      ...
      logger.info("Kitchen course auto-fired", {
        ticketId: ticket.id,
        orderId,
      ...
      await writeAudit({
        ...
        metadata: {
          orderId,
          courseNumber: ticket.course?.courseNumber ?? null,
        },
      });
    }
  },
...
eventBus.subscribe(
  "kitchen.ticket.updated",
  async ({ event, tenantId, branchId, context }) => {
