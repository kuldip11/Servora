...
export const ticketService = {
  async getQueueForCurrentBranch(auth: AuthContext, stationId?: string) {
    ...
    if (stationId) {
      const station = await stationRepository.findById(auth.tenantId, stationId);
      if (!station || station.branchId !== branchId) throw ticketNotFound(stationId);
    }
  ...
  async updateStatus(auth: AuthContext, logger: Logger, ticketId: string, newStatus: KitchenTicketStatus) {
    ...
    const updated = await ticketRepository.setStatus(auth.tenantId, ticketId, newStatus, timestampFieldsFor(newStatus));
    if (!updated) throw ticketNotFound(ticketId);
    const detailed = await publishDetailedTicket(auth, ticketId, "kitchen.ticket.updated");

    if (current.status === "HELD" && newStatus === "FIRED") {
      await deductTicketWhenFired(auth, detailed);
      await writeAudit({
        tenantId: auth.tenantId, userId: auth.userId, branchId: current.branchId,
        requestId: auth.requestId, ipAddress: auth.ipAddress,
        action: "KITCHEN_COURSE_MANUALLY_FIRED", entity: "kitchen_ticket", entityId: ticketId,
        metadata: { orderId: detailed.orderId, courseNumber: detailed.course?.courseNumber ?? null },
      });
    }
    logger.info("Kitchen ticket status updated", { ticketId, from: current.status, to: newStatus });
    return detailed;
  },
};
