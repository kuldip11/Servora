...
describe("ticket repository", () => {
  ...
  it("reports whether an order already contains the requested course", async () => {
    ...
    await expect(ticketRepository.hasCourseNumber("t1", "o1", 1)).resolves.toBe(
      true,
    );
    await expect(ticketRepository.hasCourseNumber("t1", "o1", 2)).resolves.toBe(
      false,
    );
  });
  ...
  it("holds later courses until the immediately prior course is served", async () => {
    ...
    await expect(
      ticketRepository.shouldHoldCourse("t1", "o1", 2),
    ).resolves.toBe(true);
    findMany.mockResolvedValue([
      { id: "c1", status: "SERVED", course: { courseNumber: 1 } },
      { id: "c2", status: "HELD", course: { courseNumber: 2 } },
    ...
    await expect(
      ticketRepository.findAutoFireableHeldTickets("t1", "o1"),
