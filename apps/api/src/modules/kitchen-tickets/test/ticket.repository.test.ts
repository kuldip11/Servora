...
const {db}=vi.hoisted(()=>({db:{ query: { kitchenTickets: { findMany, findFirst } }, update: vi.fn() }}));
vi.mock('../../../db', () => ({ db }));
...
describe('ticket repository', () => {
  it('queries the current tenant/branch queue and ticket by id', async () => {
    ...
    findFirst.mockResolvedValue({ id: 't1', tenantId: 't1', branchId: 'b1', status: 'READY' });
    await expect(ticketRepository.getQueue('t1', 'b1')).resolves.toEqual([{ id: 't1', status: 'READY' }]);
    await expect(ticketRepository.findById('t1', 't1')).resolves.toMatchObject({ id: 't1' });
  });
  it('updates status with timestamp fields and returns the row', async () => {
    const updated = { id: 't1', status: 'READY' };
    db.update.mockReturnValue({ set: vi.fn().mockReturnValue({ where: vi.fn().mockReturnValue({ returning: vi.fn().mockResolvedValue([updated]) }) }) });
    await expect(ticketRepository.setStatus('t1', 't1', 'READY', { readyAt: new Date() })).resolves.toEqual(updated);
  });
