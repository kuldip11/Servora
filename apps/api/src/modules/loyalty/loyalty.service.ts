...
const normalizedTier = (input: LoyaltyTierInput) => {
  const hasPercent =
...
const assertTierTenant = async (
  ...
  if (!(await loyaltyRepository.findApplicableTier(tenantId, tierId))) {
    throw new ValidationError(
      "Loyalty tier does not belong to this tenant or its organization",
    );
  }
};
...
export const loyaltyService = {
  ...
  async createTier(auth: AuthContext, input: LoyaltyTierInput) {
    ...
    const row = await loyaltyRepository.createTier({
      tenantId: auth.tenantId,
      ...normalizedTier(input),
    });
    await writeAudit({
      tenantId: auth.tenantId,
      userId: auth.userId,
  ...
  async updateTier(
    ...
    const row = await loyaltyRepository.updateTier(
      auth.tenantId,
      id,
      normalizedTier(merged),
    );
  ...
  async createCustomer(auth: AuthContext, input: CustomerInput) {
    ...
    const existingIdentity = phone
      ? await loyaltyRepository.findOrganizationCustomerIdentity(
          auth.tenantId,
          phone,
        )
      : null;
  ...
  async updateCustomer(
    ...
    if (patch.loyaltyTierId !== undefined)
      await assertTierTenant(auth.tenantId, patch.loyaltyTierId);
    ...
    const organizationCustomerId = normalizedPhone
      ? ((await loyaltyRepository.findOrganizationCustomerIdentity(
        ...
        existing.organizationCustomerId ??
        existing.id)
      : null;
    const row = await loyaltyRepository.updateCustomer(auth.tenantId, id, {
      ...
      ...(patch.loyaltyTierId !== undefined
        ? { loyaltyTierId: patch.loyaltyTierId }
        : {}),
    });
    ...
    await writeAudit({
      ...
      entity: "customer",
      entityId: id,
      metadata: { loyaltyTierId: patch.loyaltyTierId },
    });
    return row;
  },
};
