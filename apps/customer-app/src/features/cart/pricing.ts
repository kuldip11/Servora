...
const findOption = (item: CustomerMenuItem, optionId: string) => {
  return item.modifierGroupLinks
    .flatMap(({ group }) => group.options)
    .find((option) => option.id === optionId);
};

export const getLineUnitPrice = (line: CartLine) => {
  ...
  const optionPrice = (selection: SelectedOption) => {
    ...
    const scoped = line.variantId
      ? option?.variantPrices?.find(
          (value) => value.variantId === line.variantId,
        )
      : undefined;
    return (
      Number(scoped?.additionalPrice ?? option?.additionalPrice ?? 0) *
      selection.quantity
    );
  };
  const whole = line.selectedOptions
    .filter(
      (selection) => !selection.zoneLabel || selection.zoneLabel === "WHOLE",
    )
    .reduce((sum, selection) => sum + optionPrice(selection), 0);
  const zoned = line.selectedOptions.filter(
    (selection) => selection.zoneLabel && selection.zoneLabel !== "WHOLE",
  );
  ...
  for (const selection of zoned)
    byZone.set(
      selection.zoneLabel!,
      (byZone.get(selection.zoneLabel!) ?? 0) + optionPrice(selection),
    );
  const zoneTotals = [...byZone.values()];
  const rule = line.item.zonePricingRule ?? "HIGHER";
  const zoneModifierTotal =
    !line.item.supportsZones || !zoneTotals.length
      ? zoned.reduce((sum, selection) => sum + optionPrice(selection), 0)
      : rule === "HIGHER"
        ? Math.max(...zoneTotals)
        : rule === "AVERAGE"
          ? zoneTotals.reduce((sum, value) => sum + value, 0) /
            zoneTotals.length
          : zoneTotals.reduce((sum, value) => sum + value * 0.5, 0);
  return (
    Number(variant?.price ?? line.item.basePrice) + whole + zoneModifierTotal
  );
};

export const getLineSubtotal = (line: CartLine) => {
...
export const getCartSummary = (cart: CartLine[]): CartSummary => {
...
export const getCartSubtotal = (cart: CartLine[]) => {
  return getCartSummary(cart).subtotal;
};

export const getCartTax = (cart: CartLine[]) => {
...
export const getItemCount = (cart: CartLine[]) => {
...
export const getCartLineKey = (
  ...
  return JSON.stringify({
    ...
    selectedOptions: [...line.selectedOptions].sort((a, b) =>
      `${a.optionId}:${a.zoneLabel ?? "WHOLE"}`.localeCompare(
        `${b.optionId}:${b.zoneLabel ?? "WHOLE"}`,
      ),
    ),
