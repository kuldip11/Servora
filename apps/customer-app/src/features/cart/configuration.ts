...
export const validateItemConfiguration = (
  ...
  for (const selection of selectedOptions) {
    ...
    const zone = item.supportsZones
      ? (selection.zoneLabel ?? "WHOLE")
      : "WHOLE";
    ...
    if (current.some((value) => value.optionId === selection.optionId))
      return `${option.name} was selected more than once in ${zone.toLowerCase()}.`;
  ...
  for (const { group } of item.modifierGroupLinks) {
    ...
    for (const zone of requiredZones) {
      ...
      if (selected.length < group.minSelections)
        return `Choose at least ${group.minSelections} option(s) from ${group.name}${item.supportsZones ? ` for the ${zone.toLowerCase()} side` : ""}.`;
...
export const canAddItemConfiguration = (
  item: CustomerMenuItem,
  variantId: string | undefined,
  selectedOptions: SelectedOption[],
): boolean => {
