...
export const SiteHeader = () => {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-[var(--border)] bg-[color:var(--surface)]/95 backdrop-blur">
      <a
        href="#main"
        className="sr-only focus:not-sr-only focus:absolute focus:left-4 focus:top-4 focus:z-50 focus:rounded-md focus:bg-[var(--primary)] focus:px-4 focus:py-2 focus:text-white"
      >
        Skip to content
      </a>
        ...
        >
          <div ref={productRef} className="relative">
            <div className="flex items-center gap-1">
              ...
              <button
                type="button"
                className="rounded p-1 hover:bg-[var(--surface-secondary)]"
            ...
            {productOpen && (
              <div
                id="product-menu"
                className="absolute left-1/2 top-full w-[700px] -translate-x-1/2 pt-4"
                role="menu"
              >
                <div className="grid grid-cols-2 gap-2 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 shadow-[var(--shadow-md)]">
                  {modules.map((item) => (
                    <Link
                      key={item.slug}
                      ...
                      onClick={closeMenus}
                      role="menuitem"
                      className="rounded-xl p-3 hover:bg-[var(--primary-surface)] focus:outline-none focus:ring-2 focus:ring-[var(--primary)]"
                    >
          ...
          <div ref={appsRef} className="relative">
            ...
            {appsOpen && (
              <div
                ...
                role="menu"
              >
                <div className="grid gap-1 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-3 shadow-[var(--shadow-md)]">
                  {servoraApps.map((app) => {
                    const Icon = app.icon;
                    return (
                      <Link
                      ...
                      >
                        <span className="mt-0.5 rounded-lg bg-[var(--primary-surface)] p-2 text-[var(--primary)]">
          ...
          <Link
            onClick={() =>
              track({ event: "nav_cta_click", location: "header" })
            }
            href="/book-a-demo"
            className="rounded-lg bg-[var(--primary)] px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-[var(--primary-hover)] focus:outline-none focus:ring-2 focus:ring-[var(--primary)] focus:ring-offset-2"
          >
            Book a Demo
          </Link>
        </nav>
        ...
        >
          <div className="grid gap-1">
            <Link
              onClick={closeMenus}
              href="/product"
              className="rounded-lg px-3 py-2.5 text-sm font-semibold hover:bg-[var(--primary-surface)]"
            >
              Product overview
            </Link>
            {modules.map((item) => (
            ...
            <Link
              ...
              href="/book-a-demo"
              className="mt-2 rounded-lg bg-[var(--primary)] px-4 py-3 text-center text-sm font-semibold text-white"
            >
              Book a Demo
            </Link>
          </div>
