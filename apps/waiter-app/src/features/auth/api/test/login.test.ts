...
describe('auth API', () => {
  it('posts credentials and returns login data', async () => {
    const payload = { accessToken: 'a', refreshToken: 'r', expiresIn: 3600, user: { id: 'u1' } };
    vi.mocked(apiClient.post).mockResolvedValue({ data: { data: payload } } as any);
  ...
  it('returns available memberships', async () => {
    vi.mocked(apiClient.get).mockResolvedValue({ data: { data: [{ id: 'm1' }] } } as any);
