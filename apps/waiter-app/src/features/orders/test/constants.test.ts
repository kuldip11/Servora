...
import { orderKeys, ORDERS_POLL_INTERVAL_MS, ORDER_DETAIL_POLL_INTERVAL_MS, STATUS_CONFIG, TICKET_STATUS_LABEL } from '../constants';
...
describe('orders constants', () => {
  ...
  it('exposes billing and ticket status labels', () => {
    ...
    expect(TICKET_STATUS_LABEL).toEqual({
      FIRED: 'Waiting', PREPARING: 'Cooking', READY: 'Ready', SERVED: 'Served',
    });
  });
});
