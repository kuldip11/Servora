...
interface Props {
  ...
  onSeatShares?: ((itemId: string, current: Array<{ seatLabel: string; shareRatio: number | string }>) => void) | undefined;
  ...
  return (
    ...
    <Card padding="none" className="mx-4 mt-4 rounded-2xl overflow-hidden">
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    {item.weightQuantity != null && (
                      <p className="text-xs text-text-secondary">{Number(item.weightQuantity)} {item.weightUnit ?? ""}</p>
                    )}
                    ...
                    {item.itemStatus === "VOIDED" && (
                      <p className="text-xs text-danger no-underline">Voided · {item.voidedReason}</p>
                    )}
                    ...
                    {item.refiresOrderItemId && (
                      <p className={`text-xs font-semibold ${item.refireType === "REFILL" ? "text-success" : "text-warning"}`}>
                        {item.refireType === "REFILL" ? "REFILL · INCLUDED" : "REFIRE"} replacement of #{item.refiresOrderItemId.slice(-6)}
                      </p>
                    )}
                    {item.itemStatus === "COMPED" && (
                      <p className="text-xs text-success">Comped · {item.compedReason}</p>
                    )}
                    {item.itemStatus === "ACTIVE" && (onAdjust || onRefire || onRefill || onSeatShares) && (canVoid || canComp || onRefire || onRefill || onSeatShares) && (
                      <div className="flex flex-wrap gap-2 mt-2">
                        {onRefill && item.comboSlotOption?.isUnlimitedRefill && <Button size="sm" variant="success" onClick={() => onRefill(item.id)}><RotateCcw className="h-3.5 w-3.5" /> Refill</Button>}
                        {onRefire && <Button size="sm" variant="secondary" onClick={() => onRefire(item.id)}><RotateCcw className="h-3.5 w-3.5" /> Refire</Button>}
                        {onSeatShares && <Button size="sm" variant="secondary" onClick={() => onSeatShares(item.id, item.seatShares ?? [])}>Split item</Button>}
                        {canVoid && <Button size="sm" variant="danger" onClick={() => onAdjust?.(item.id, "void")}>Void</Button>}
                        {canComp && <Button size="sm" variant="secondary" onClick={() => onAdjust?.(item.id, "comp")}>Comp</Button>}
                      </div>
                    )}
                  </div>
      ...
      {ticket.status === "HELD" && onFireHeld && (
        <div className="px-4 py-3 border-t border-divider"><Button onClick={() => onFireHeld(ticket.id)} disabled={isUpdating} className="w-full rounded-2xl">Fire Course Now</Button></div>
      )}
      {ticket.status === "READY" && (
