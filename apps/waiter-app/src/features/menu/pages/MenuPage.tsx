...
export function MenuPage({ onBack, onOrderPlaced, existingOrderId }: Props) {
  ...
  useEffect(() => {
    if (categories?.length && !activeCategory) setActiveCategory(categories[0]?.id ?? null);
  ...
  function handleItemTap(item: any) {
    ...
    if (hasOptions) { setCustomising({ item }); return; }
    addOrIncrementItem({
  ...
  function addOrIncrementItem(newItem: CartItem) {
    setCart((prev) => {
      ...
      if (ex) return prev.map((c) => cartItemKey(c) === key ? { ...c, quantity: c.quantity + newItem.quantity } : c);
      return [...prev, newItem];
    });
  ...
  function updateQty(key: string, delta: number) {
    setCart((prev) =>
      prev.map((c) => cartItemKey(c) === key ? { ...c, quantity: c.quantity + delta } : c)
  ...
  function handleSubmit() {
    ...
    if (isAddingToExisting) {
      const validated = addOrderItemsSchema.safeParse({ items, ...(orderNotes ? { notes: orderNotes } : {}) });
      if (!validated.success) { toast({ title: 'Please check the order details', tone: 'danger' }); return; }
      addItemsMutation.mutate(
        { orderId: existingOrderId!, items, ...(orderNotes ? { notes: orderNotes } : {}) },
      ...
      const orderInput: CreateOrderInput = {
        ...
        ...(validated.data.customerId ? { customerId: validated.data.customerId } : {}),
  ...
  const activeItems: any[] = (menuSearch.length >= 2
    ? allItems.filter((i: any) => i.isAvailable && i.name.toLowerCase().includes(menuSearch.toLowerCase()))
    : categories?.find((c: any) => c.id === activeCategory)?.menuItems?.filter((i: any) => i.isAvailable) ?? []
  ).filter((i: any) => foodTypeFilter === 'ALL' || i.foodType === foodTypeFilter);
    ...
    <div className="flex flex-col h-screen bg-surface-secondary">
      ...
      {!isAddingToExisting && (
        <OrderOptionsPanel
          ...
          orderType={orderType}
          onOrderTypeChange={(t) => { setOrderType(t); setTableId(''); }}
          ...
          onClearCustomer={() => { setCustomerId(''); setCustomerName(''); }}
          customerSearch={customerSearch}
          onCustomerSearchChange={setCustomerSearch}
          customerResults={customerResults}
          onSelectCustomer={(id, name) => { setCustomerId(id); setCustomerName(name); setCustomerSearch(''); }}
        />
      )}
      ...
      {totalItems > 0 && (
        <div className="fixed bottom-0 left-0 right-0 bg-surface border-t border-border px-4 py-4">
          <button onClick={() => setShowCart(true)}
            ...
            <span className="w-7 h-7 bg-primary-foreground/20 rounded-full flex items-center justify-center text-xs font-bold">{totalItems}</span>
      ...
      {/* Customiser modal */}
      {customising && (
        <ItemCustomiser item={customising.item} onConfirm={addOrIncrementItem} onClose={() => setCustomising(null)} />
      )}
