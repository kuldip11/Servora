...
describe('cart helpers', () => {
  it('formats flat and variant-driven prices', () => {
    ...
    expect(priceLabel({ basePrice: '0', variants: [{ price: '10' }, { price: '10' }] })).toBe('₹10.00');
    expect(priceLabel({ basePrice: '0', variants: [{ price: '10' }, { price: '15' }] })).toBe('₹10.00–₹15.00');
  });

  it('builds a stable key independent of modifier ordering', () => {
    const a = { menuItemId: 'm1', variantId: 'v1', modifiers: [
      { optionId: 'b', quantity: 2 }, { optionId: 'a', quantity: 1 },
    ] };
