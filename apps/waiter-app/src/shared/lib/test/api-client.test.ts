...
const mocks = vi.hoisted(() => ({
  createApiClient: vi.fn((config: unknown) => ({ post: vi.fn(), get: vi.fn(), __config: config })),
}));
...
describe('api client', () => {
  it('creates the shared client with waiter storage', () => {
    ...
    expect(mocks.createApiClient).toHaveBeenCalledWith(expect.objectContaining({ timeout: 15_000 }));
