...
describe('browser utility hooks', () => {
  it('persists local storage values and falls back on malformed JSON', () => {
    ...
    hook.rerender();
    expect(JSON.parse(localStorage.getItem('coverage-key')!)).toEqual({ count: 2 });
    hook.unmount();
  ...
  it('tracks media query changes and cleans up listeners', () => {
    ...
    const add = vi.fn((_: string, cb: (event: MediaQueryListEvent) => void) => { listener = cb; });
    const remove = vi.fn();
    vi.stubGlobal('matchMedia', vi.fn(() => ({ matches: false, addEventListener: add, removeEventListener: remove })));
  ...
  it('handles export success and failure states', async () => {
    ...
    download.mockImplementationOnce(() => new Promise<void>((resolve) => { resolveDownload = resolve; }));
    const hook = renderHook(() => useExportMenu());
    let promise: Promise<void>;
    act(() => {
      promise = hook.result.download('ITEM', 'csv');
    });
    hook.rerender();
    expect(hook.result.downloadingKey).toBe('ITEM-csv');
    await act(async () => {
      resolveDownload();
      await promise;
    });
    expect(hook.result.downloadingKey).toBeNull();
    expect(download).toHaveBeenCalledWith('ITEM', 'csv');

    download.mockRejectedValueOnce(new Error('download failed'));
    await act(async () => { await hook.result.download('CATEGORY', 'xlsx'); });
    expect(vi.mocked(notifyError)).toHaveBeenCalledWith(undefined, 'Failed to export CATEGORY');
    expect(hook.result.downloadingKey).toBeNull();
    hook.unmount();
  });
});
