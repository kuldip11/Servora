...
const { invalidateQueries, setQueryData, notifySuccess, notifyError } = vi.hoisted(() => ({
...
vi.mock('../../shared/lib/api-client', () => {
  const response = () => Promise.resolve({ data: { data: { updated: 1, created: 1, deleted: 1, id: 'item-1' } } });
  return {
    apiClient: { get: response, post: response, put: response, patch: response, delete: response },
...
vi.mock('../../shared/lib/realtime', () => ({
  useRealtimeEvent: vi.fn((_type: string, handler: (event: any) => void) => {
    try { handler({ type: _type, payload: { id: 'item-1', orderId: 'order-1', kitchenTickets: [], updatedAt: new Date().toISOString() } }); } catch { /* individual hooks may require richer payloads */ }
  }),
  useRealtime: vi.fn(),
...
const proxyData: any = new Proxy(
  { updated: 1, created: 1, deleted: 1, item: { id: 'item-1' }, publish: true, existingId: 'group-1' },
  { get: (target, prop) => (prop in target ? target[prop as keyof typeof target] : 1) },
);

const hookModules = import.meta.glob('../../features/**/hooks/*.ts', { eager: true }) as Record<string, Record<string, any>>;

function hookEntries() {
  return Object.entries(hookModules).flatMap(([path, module]) =>
    Object.entries(module)
      .filter(([name, value]) => /^use[A-Z]/.test(name) && typeof value === 'function' && name !== 'useExportMenu')
...
describe('feature hooks coverage', () => {
  it('creates every query/mutation/realtime hook without side effects', async () => {
    ...
    for (const { path, name, fn } of entries) {
      ...
      let result: any;
      try {
        result = fn({ id: 'item-1', itemId: 'item-1', orderId: 'order-1', branchId: 'branch-1' });
      } catch (firstError) {
        try {
          result = fn('item-1');
        } catch {
          throw new Error(`Unable to initialize ${name} from ${path}: ${String(firstError)}`);
        }
      }

      if (result && typeof result === 'object' && typeof result.mutationFn === 'function') {
        try {
          await result.mutationFn(name === 'useMenuImport' ? new File(['coverage'], 'coverage.csv') : proxyData);
        } catch {
        ...
        if (typeof result.onSuccess === 'function') {
          try { result.onSuccess(proxyData, proxyData); } catch { /* callback branch-specific data */ }
