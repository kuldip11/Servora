...
const PRIMARY_TOKENS = ['--primary', '--primary-hover', '--primary-surface', '--primary-border'];
...
const SPACING_TOKENS = ['--spacing-xs', '--spacing-sm', '--spacing-md', '--spacing-lg'];

function Swatch({ token, textOn }: { token: string; textOn?: 'light' | 'dark' }) {
  return (
    <div
      style={{
          ...
          {THEMES.map((t) => (
            <button
              ...
              style={{
                ...
                borderRadius: 'var(--radius-md)',
                border:
                  theme === t.value ? '1px solid var(--primary)' : '1px solid var(--border)',
                background: theme === t.value ? 'var(--primary-surface)' : 'var(--surface)',
                color: theme === t.value ? 'var(--primary)' : 'var(--text-primary)',
                fontSize: 14,
                fontWeight: 500,
                cursor: 'pointer',
              }}
