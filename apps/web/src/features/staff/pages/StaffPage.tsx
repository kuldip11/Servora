...
import {
  Button, Card, Modal, Input, Select, IconButton, StatusBadge,
  Page, PageHeader, Table, type Column, type StatusTone,
} from '@pos/ui';
...
export function StaffPage() {
  ...
  function handleAdd(values: CreateStaffInput) {
    addMutation.mutate({ ...values, ...(values.branchId ? { branchId: values.branchId } : {}) }, {
      onSuccess: () => {
        setShowAdd(false);
        reset({ firstName: '', lastName: '', email: '', password: '', roleId: '', branchId: undefined });
      },
    });
  }
  ...
  const columns: Column<StaffRow>[] = [
    ...
    },
    {
      id: 'email',
      header: 'Email',
      cell: (member) => <span className="text-text-secondary">{member.email}</span>,
    },
    ...(showBranchPicker
      ? [{
          id: 'branch',
          ...
          cell: (member: StaffRow) =>
            member.branch?.name
              ? <span className="text-text-secondary">{member.branch.name}</span>
      ...
      : []),
    {
      ...
      cell: (member) =>
        member.userRoles?.[0]?.role?.name
          ? <StatusBadge tone="info" dot={false} label={member.userRoles[0].role.name} />
          : null,
    },
    {
      id: 'status',
      header: 'Status',
      cell: (member) => (
        <StatusBadge tone={STATUS_TONES[member.status] ?? 'neutral'} label={member.status} />
      ),
    },
    {
      id: 'actions',
      ...
      cell: (member) => (
        <div className="flex items-center justify-end gap-1">
          {has('staff:update') && (member.status === 'ACTIVE' ? (
            <IconButton
              icon={UserX}
              size="sm"
              aria-label="Deactivate"
              title="Deactivate"
              onClick={() => updateStatusMutation.mutate({ id: member.id, status: 'INACTIVE' })}
            />
          ) : (
            <IconButton
              icon={UserCheck}
          ...
          {has('staff:deactivate') && <IconButton
            icon={Trash2}
            size="sm"
            aria-label="Remove staff member"
            onClick={() => {
              if (confirm('Remove this staff member?')) deleteMutation.mutate(member.id);
            }}
          />}
        </div>
      ),
  ...
  return (
    <Page>
      <PageHeader
        ...
        actions={
          has('staff:create') && <Button onClick={() => setShowAdd(true)}>
      ...
      <Card padding="none" className="overflow-hidden">
        <Table
          ...
          emptyAction={
            has('staff:create') && <Button onClick={() => setShowAdd(true)}><Plus className="w-4 h-4" /> Add Staff</Button>
          }
        />
      </Card>
      ...
      <Modal open={showAdd} onClose={() => setShowAdd(false)} title="Add Staff Member">
        <form onSubmit={handleSubmit(handleAdd)} className="space-y-4">
          ...
          <Select label="Role" options={roleSelectOptions} error={errors.roleId?.message} {...register('roleId')} />
          {showBranchPicker && <Select label="Branch" options={branchSelectOptions} error={errors.branchId?.message} {...register('branchId')} />}
          <div className="flex gap-2 justify-end">
            <Button type="button" variant="secondary" onClick={() => setShowAdd(false)}>Cancel</Button>
            <Button type="submit" loading={addMutation.isPending}>Add Staff</Button>
          </div>
        </form>
      </Modal>
    </Page>
