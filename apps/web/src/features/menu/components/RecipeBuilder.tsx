...
export function RecipeBuilder({ itemId }: { itemId: string }) {
  ...
  function handleSave() {
    const ingredients = rows
      ...
      .map((r) => ({ inventoryItemId: r.inventoryItemId, quantity: parseFloat(r.quantity) || 0, unit: r.unit, isOptional: r.isOptional }));
  ...
  function addRow() {
    const first = inventoryItems?.find((inv) => !rows.some((r) => r.inventoryItemId === inv.id));
    if (!first) {
      notifyError(undefined, rows.length ? 'All inventory items are already in this recipe' : 'No inventory items yet — add some from the Inventory page first');
      return;
    }
    setDirty(true);
    setRows((prev) => [...prev, { inventoryItemId: first.id, quantity: '1', unit: first.unit, isOptional: false }]);
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1.5">
        <span className="text-sm font-medium text-text-primary">Ingredients (auto-deducts inventory when ordered)</span>
        <button
          onClick={handleSave}
      ...
      {isLoading ? (
        ...
        <p className="text-xs text-text-disabled">
          No inventory items set up yet for this branch — add some from the Inventory page, then come back to link them here.
        </p>
      ) : (
        <div className="space-y-2">
          {rows.map((row, i) => {
            const inv = invMap.get(row.inventoryItemId);
            const short = inv && parseFloat(row.quantity || '0') > 0 && inv.currentStock < parseFloat(row.quantity || '0');
            return (
              <div key={i} className="flex items-center gap-2">
                ...
                >
                  {inventoryItems.map((inv2) => (
                    <option key={inv2.id} value={inv2.id} disabled={rows.some((r, idx) => idx !== i && r.inventoryItemId === inv2.id)}>
                      {inv2.name}
                    </option>
                  ))}
                </select>
                <input
                  type="number" min="0" step="0.001" value={row.quantity}
                ...
                <select
                  value={row.unit}
                  onChange={(e) => updateRow(i, { unit: e.target.value as InventoryUnit })}
                ...
                <label className="flex items-center gap-1 text-xs text-text-secondary whitespace-nowrap">
                  <input type="checkbox" checked={row.isOptional} onChange={(e) => updateRow(i, { isOptional: e.target.checked })} />
                  optional
                </label>
                {!row.isOptional && (
                  short
                    ? <span title="Low stock"><AlertTriangle className="w-4 h-4 text-warning shrink-0" /></span>
                    : <CheckCircle2 className="w-4 h-4 text-success shrink-0" />
                )}
                <button onClick={() => removeRow(i)} aria-label={`Remove ingredient ${i + 1}`} className="p-1 text-text-disabled hover:text-danger shrink-0">
      ...
      <p className="text-xs text-text-disabled mt-2">
        Every non-optional ingredient is deducted automatically when this item is ordered. If stock runs out for any
        of them, the item flips to "Out of Stock" on its own — no need to toggle it by hand.
      </p>
    </div>
  );
}
