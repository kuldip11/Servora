...
export const MenusSection = () => {
  ...
  function startEditing(menu: Menu) {
    ...
    setFulfillmentTypes(
      menu.availableFulfillmentTypes ?? [...FULFILLMENT_TYPES],
    );
    setBranchIds(
      menu.availableBranchIds ?? branches.map((branch) => branch.id),
    );
    setEffectiveFrom(
      menu.effectiveFrom
        ? new Date(menu.effectiveFrom).toISOString().slice(0, 16)
        : "",
    );
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <Modal
        ...
        title={`Availability — ${editing?.name ?? "menu"}`}
        size="sm"
      >
        <form
          className="space-y-5"
            ...
            updateMenu.mutate(
              {
                ...
                input: {
                  availableChannels:
                    channels.length === MENU_CHANNELS.length
                      ? null
                      : (channels as Menu["availableChannels"]),
                  availableFulfillmentTypes:
                    fulfillmentTypes.length === FULFILLMENT_TYPES.length
                      ? null
                      : (fulfillmentTypes as Menu["availableFulfillmentTypes"]),
                  availableBranchIds:
                    branchIds.length === branches.length ? null : branchIds,
        ...
        >
          <ScopeChoices
            ...
            values={channels}
            onToggle={(value) => toggle(value, channels, setChannels)}
          />
          <ScopeChoices
            ...
            onToggle={(value) =>
              toggle(value, fulfillmentTypes, setFulfillmentTypes)
            }
          />
          <ScopeChoices
            label="Branches"
            ...
            values={branchIds}
            onToggle={(value) => toggle(value, branchIds, setBranchIds)}
          />
          {branchIds.length < branches.length && (
            <p className="rounded bg-warning-surface px-3 py-2 text-xs text-warning">
              Branch-specific items assigned to this menu are reachable only
              where the item’s own branch and this menu’s selected branches
              overlap.
            </p>
          )}
