...
function toDraft(o: MenuItemBranchOverride | undefined): RowDraft {
  return {
    ...
    prepTimeMinutes: o?.prepTimeMinutes != null ? String(o.prepTimeMinutes) : '',
    status: o?.status ?? '',
...
export function BranchOverridesPanel({ itemId, basePrice, baseTaxRate, basePrepTimeMinutes }: Props) {
  ...
  const overrideByBranch = new Map((overrides ?? []).map((o) => [o.branchId, o]));
  ...
  if (isLoading) return <p className="text-xs text-text-disabled">Loading branches…</p>;
  if (!branches?.length) return <p className="text-xs text-text-disabled">No branches set up yet.</p>;

  return (
    <div>
      <label className="text-sm font-medium text-text-primary mb-1.5 block">
        Per-branch overrides{' '}
        <span className="font-normal text-text-disabled">(price, tax, prep time, status, or hide — leave blank to use the default above)</span>
      </label>
            ...
            <div key={b.id} className="border border-border rounded-md p-2.5">
              <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-3 text-xs text-text-secondary shrink-0">
                  {!isEditing && (
                    <>
                      <span>₹{override?.price != null ? override.price : basePrice}</span>
                      <span>
                        {override?.isHidden
                          ? 'Hidden'
                          : STATUS_OPTIONS.find((o) => o.value === (override?.status ?? 'ACTIVE'))?.label}
                      </span>
                      {override && (
                        <button
                      ...
                      <button
                        onClick={() => { setDraft(toDraft(override)); setEditingBranchId(b.id); }}
              ...
              {isEditing && (
                <div className="mt-2 space-y-2">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <select
                      value={draft.status}
                      onChange={(e) => setDraft((d) => ({ ...d, status: e.target.value as MenuItemStatus | '' }))}
                  ...
                  <input
                    ...
                    onChange={(e) => setDraft((d) => ({ ...d, availabilityReason: e.target.value }))}
                    aria-label="Branch override reason"
                    className="w-full px-2 py-1.5 text-sm border border-border rounded-md"
                  />
                  <div className="flex gap-2 justify-end">
                    <button onClick={() => setEditingBranchId(null)} className="text-xs text-text-secondary hover:text-text-primary px-2 py-1">
                      Cancel
                    </button>
                    <button
                      onClick={() => saveMutation.mutate({ branchId: b.id, input: draft }, { onSuccess: () => setEditingBranchId(null) })}
