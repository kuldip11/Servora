...
import type { MenuItemSchedule, MenuItemScheduleType, MenuItemStatus } from '@pos/types';
...
function describe(s: MenuItemSchedule): string {
  switch (s.scheduleType) {
    ...
    case 'SPECIFIC_DATE':
      return s.endDate && s.endDate !== s.startDate ? `${s.startDate} – ${s.endDate}` : `${s.startDate}`;
    case 'HOLIDAY':
  ...
  return (
    <div>
      <span className="text-sm font-medium text-text-primary mb-1.5 block">
        Availability windows <span className="font-normal text-text-disabled">(item only shows/hides for these — everything else uses its base status)</span>
      </span>

      {isLoading ? (
        <p className="text-xs text-text-disabled">Loading…</p>
      ) : !schedules?.length && !draft ? (
        <p className="text-xs text-text-disabled mb-2">No schedules yet — this item follows its base status all the time.</p>
      ) : (
        <div className="space-y-1.5 mb-2">
          {schedules?.map((s) => (
            <div key={s.id} className="flex items-center justify-between gap-2 px-3 py-1.5 bg-surface-secondary rounded-md text-xs">
              <div className="flex items-center gap-2 text-text-secondary">
                <Clock className="w-3.5 h-3.5 text-text-disabled" />
                <span>{describe(s)}</span>
                <span className="text-text-disabled">→</span>
                <span className="font-medium">{STATUS_OPTIONS.find((o) => o.value === s.statusDuringPeriod)?.label}</span>
              </div>
              <button onClick={() => deleteMutation.mutate(s.id)} aria-label={`Remove schedule: ${describe(s)}`} className="text-text-disabled hover:text-danger">
      ...
      {draft ? (
        <div className="border border-border rounded-md p-3 space-y-2">
          <select
            ...
            onChange={(e) => setDraft({ ...draft, scheduleType: e.target.value as MenuItemScheduleType })}
          ...
          {(draft.scheduleType === 'DAILY' || draft.scheduleType === 'WEEKLY') && (
            <div className="flex items-center gap-2">
              ...
              <input type="time" value={draft.startTime} onChange={(e) => setDraft({ ...draft, startTime: e.target.value })}
                aria-label="Start time" className="px-2 py-1.5 text-sm border border-border rounded-md" />
              ...
              <input type="time" value={draft.endTime} onChange={(e) => setDraft({ ...draft, endTime: e.target.value })}
                aria-label="End time" className="px-2 py-1.5 text-sm border border-border rounded-md" />
            </div>
          )}
          ...
          {draft.scheduleType === 'SPECIFIC_DATE' && (
            <div className="flex items-center gap-2">
              <input type="date" value={draft.startDate} onChange={(e) => setDraft({ ...draft, startDate: e.target.value })}
                aria-label="Start date" className="px-2 py-1.5 text-sm border border-border rounded-md" />
              <span className="text-text-disabled text-xs">to</span>
              <input type="date" value={draft.endDate} onChange={(e) => setDraft({ ...draft, endDate: e.target.value })}
                aria-label="End date" className="px-2 py-1.5 text-sm border border-border rounded-md" />
            </div>
          )}
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-text-secondary">Status during this window:</span>
            <select
              value={draft.statusDuringPeriod}
              onChange={(e) => setDraft({ ...draft, statusDuringPeriod: e.target.value as MenuItemStatus })}
          ...
          <div className="flex gap-2 justify-end pt-1">
            <button onClick={() => setDraft(null)} className="text-xs text-text-secondary hover:text-text-primary px-2 py-1">Cancel</button>
            <button
