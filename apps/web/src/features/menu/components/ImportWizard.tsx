...
import { menuImportService, type ValidatedRow, type ValidateImportResponse } from '../services/menu-import.service';
import { useValidateMenuImport, useCommitMenuImport } from '../hooks/useMenuImport';
...
const PREVIEW_COLUMNS: Column<ValidatedRow>[] = [
    ...
    cell: (r) => (
      <span
        className={`px-1.5 py-0.5 rounded text-[11px] font-medium ${
          r.action === 'insert' ? 'bg-primary-surface text-primary' : 'bg-warning-surface text-warning'
        }`}
      >
        {r.action}
      </span>
  ...
  { id: 'status', header: 'Status', cell: (r) => <span className="text-text-secondary">{r.data.status}</span> },
];
      ...
      footer={
        file && result ? (
          <>
            ...
            <Button onClick={handleCommit} loading={commitMutation.isPending} disabled={result.validCount === 0}>
              {commitMutation.isPending ? 'Importing…' : `Import ${result.validCount} item(s)`}
            </Button>
          </>
        ) : undefined
      }
    >
      <div className="space-y-4">
        <div className="flex items-center gap-3 text-sm">
          ...
          <button onClick={() => downloadTemplate('csv')} className="flex items-center gap-1 text-primary hover:text-primary-hover font-medium">
            <Download className="w-3.5 h-3.5" /> CSV template
          </button>
          <button onClick={() => downloadTemplate('xlsx')} className="flex items-center gap-1 text-primary hover:text-primary-hover font-medium">
        ...
        {!file && (
          ...
          >
            ...
            <span className="text-sm font-medium">Click to upload a CSV or Excel file</span>
          </button>
        )}
        <input ref={fileInputRef} type="file" accept=".csv,.xlsx,.xls" className="hidden" onChange={handleFileChange} />

        {validateMutation.isPending && <p className="text-sm text-text-secondary">Validating…</p>}
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-4 text-sm">
              <span className="flex items-center gap-1 text-success font-medium">
                <CheckCircle2 className="w-4 h-4" /> {result.validCount} ready to import
              </span>
              {result.errors.length > 0 && (
                <span className="flex items-center gap-1 text-danger font-medium">
                  <AlertCircle className="w-4 h-4" /> {result.errors.length} row error(s)
                </span>
              )}
            </div>
            ...
            {result.errors.length > 0 && (
              <div className="border border-danger/20 bg-danger-surface rounded-md max-h-40 overflow-y-auto">
                {result.errors.map((e, i) => (
                  <div key={i} className="px-3 py-1.5 text-xs text-danger border-b border-danger/20 last:border-0">
