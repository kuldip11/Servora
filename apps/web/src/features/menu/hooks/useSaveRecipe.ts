...
import { menuRecipesService, type RecipeIngredientInput } from '../services/menu-recipes.service';
import { menuKeys } from '../query-keys';
import { inventoryKeys } from '../../inventory/query-keys';

export function useSaveRecipe(itemId: string) {
  return useMutation({
    mutationFn: (ingredients: RecipeIngredientInput[]) => menuRecipesService.save(itemId, ingredients),
    onSuccess: () => {
