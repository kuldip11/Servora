...
export function AddItemsModal({ orderId, onClose }: { orderId: string; onClose: () => void }) {
  ...
  const allItems: MenuItem[] = categories?.flatMap((c: MenuCategory) => c.menuItems ?? []) ?? [];
  const visibleCategories = categories?.map((cat: MenuCategory) => ({
    ...cat,
    menuItems: (cat.menuItems ?? []).filter((it) => foodTypeFilter === 'ALL' || it.foodType === foodTypeFilter),
  }));
  ...
  function handleSubmit() {
    ...
    if (!parsed.success) {
      setValidationError(parsed.error.issues[0]?.message ?? 'Please review the items.');
      return;
    }
    setValidationError('');
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div>
            ...
            {FOOD_TYPE_FILTERS.map((f) => (
              <button
                ...
                className={`px-2.5 py-1 rounded-full text-xs font-semibold border transition-colors ${
                  foodTypeFilter === f.value ? 'border-primary bg-primary-surface text-primary' : 'border-border text-text-secondary'
                }`}
              >
                {f.label}
              </button>
          ...
          <div className="space-y-4 max-h-80 overflow-y-auto pr-1">
            {!allItems.length && (
              <p className="text-sm text-text-disabled text-center py-6">No menu items yet.</p>
            )}
                    ...
                    {cat.menuItems.map((item) => (
                      ...
                      >
                        <span className="text-sm text-text-primary group-hover:text-primary-hover">
                          {item.name}
                          {(item.variants?.length > 0 || (item.modifierGroupLinks?.length ?? 0) > 0) && (
                            <span className="text-xs text-text-disabled ml-1.5">Options ▾</span>
                          )}
                        </span>
        ...
        <div className="flex flex-col">
          ...
          {!items.length ? (
            ...
            <div className="flex-1 space-y-2 overflow-y-auto max-h-64 pr-1">
              {items.map((item) => {
                ...
                return (
                  <div key={key} className="flex items-start gap-2 bg-surface-secondary rounded-md p-2">
                    <div className="flex-1 min-w-0">
                      <span className="text-sm text-text-primary truncate block">{item.menuItemName}</span>
                      {item.variantName && <span className="text-xs text-text-secondary">{item.variantName}</span>}
                      {item.modifiers.map((m) => (
                        <span key={m.optionId} className="text-xs text-text-disabled block">
                          + {m.name}{m.quantity > 1 ? ` ×${m.quantity}` : ''}
                        </span>
                      ))}
                      {item.chefNotes && <span className="text-xs text-primary block">📝 {item.chefNotes}</span>}
                    </div>
                    ...
                    <button onClick={() => updateQty(key, -item.quantity)} aria-label={`Remove ${item.menuItemName} from order`} className="text-text-disabled hover:text-danger flex-shrink-0">
          ...
          <div className="mt-4 pt-4 border-t border-divider">
            ...
            <div className="flex items-center justify-between mb-4">
              <span className="text-sm text-text-secondary">New items total</span>
              <span className="text-xl font-bold text-text-primary">{formatCurrency(total)}</span>
            </div>
            {validationError && <p className="text-xs text-danger text-center mb-2">{validationError}</p>}
            <Button
              className="w-full"
