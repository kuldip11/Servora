 ...
 */
export function ItemCustomizerModal({ item, existingCartItem, onConfirm, onClose }: Props) {
  const hasVariants = item.variants?.length > 0;
  const groups: ModifierGroup[] = (item.modifierGroupLinks ?? []).map((link) => link.group);
  ...
  const priceBeforeModifiers = selectedVariant ? Number(selectedVariant.price) : basePrice;
      ...
      if (group.selectionType === 'SINGLE') {
        ...
        return {
          ...
          [group.id]: [{
            ...
            name: option.name, price: Number(option.additionalPrice), quantity: 1,
          }],
        };
      }
  ...
  function setOptionQuantity(group: ModifierGroup, option: ModifierOption, qty: number) {
    setSelections((prev) => {
      const current = prev[group.id] ?? [];
  ...
  function handleConfirm() {
    ...
    if (!parsed.success) {
      setValidationError(parsed.error.issues[0]?.message ?? 'Please review the item options.');
      return;
    }
    setValidationError('');
    onConfirm({
      ...
      basePrice,
      ...(variantId && { variantId }),
      ...(selectedVariant?.name !== undefined && { variantName: selectedVariant.name }),
  ...
  return (
    <Dialog
      ...
      footer={
        ...
        <Button onClick={handleConfirm} disabled={!!unmetGroup} className="w-full justify-between">
    ...
    >
      <div className="space-y-5">
        ...
        {hasVariants && (
          <div>
            <p className="text-xs font-semibold text-text-secondary uppercase tracking-wide mb-2">Size / Variant</p>
            <div className="space-y-2">
              {item.variants.map((v) => (
                <button key={v.id} onClick={() => setVariantId(v.id)}
                  ...
                  <span className="text-sm text-text-secondary">{formatCurrency(Number(v.price))}</span>
                </button>
              ))}
            </div>
          </div>
                ...
                {group.options.filter((o) => o.isAvailable).map((option) => {
                  ...
                  return (
                    <div key={option.id}
                      ...
                      <button onClick={() => !disabled && selectOption(group, option)} disabled={disabled}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-text-secondary min-w-[3.5rem] text-right">
                          {Number(option.additionalPrice) > 0 ? `+${formatCurrency(Number(option.additionalPrice))}` : 'Free'}
                        </span>
                      </div>
                    </div>
                  );
