...
import { Button, Badge, Card, Spinner, StatusBadge, Page, PageHeader, Breadcrumbs, Grid } from '@pos/ui';
...
const TICKET_STATUS_TONE: Record<string, 'info' | 'warning' | 'success' | 'neutral'> = {
  FIRED: 'info',
  PREPARING: 'warning',
  READY: 'success',
  SERVED: 'neutral',
    ...
    <Page>
      <PageHeader
        eyebrow={
          <Breadcrumbs
            items={[
              { label: 'Orders', href: '/orders', as: Link, linkProps: { to: '/orders' } },
      ...
      <Grid columns={{ base: 1, lg: 3 }} gap="lg">
          ...
          {tickets.map((ticket) => (
            <Card key={ticket.id}>
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-base font-semibold text-text-primary">Round {ticket.ticketNumber}</h2>
                <div className="flex items-center gap-2">
                  <StatusBadge label={ticket.status.charAt(0) + ticket.status.slice(1).toLowerCase()} tone={TICKET_STATUS_TONE[ticket.status] ?? 'neutral'} />
                  {ticket.status === 'READY' && (
                    <Button
                      variant="secondary"
                      size="sm"
                      loading={updateTicketMutation.isPending}
                      onClick={() => updateTicketMutation.mutate({ ticketId: ticket.id, status: 'SERVED' })}
                    >
              ...
              {ticket.notes && (
                <p className="text-xs text-warning bg-warning-surface rounded px-2 py-1.5 mb-3">📝 {ticket.notes}</p>
              )}

              <div className="space-y-3">
                {ticket.items?.map((item) => (
                  <div key={item.id} className="flex items-start justify-between py-2 border-b border-divider last:border-0">
                    <div>
                      <p className="text-sm font-medium text-text-primary">
                        {item.quantity}× {item.menuItemName}
                        {item.variantName && <span className="text-text-secondary font-normal"> · {item.variantName}</span>}
                      </p>
                      {item.chefNotes && (
                        <p className="text-xs text-warning mt-0.5">📝 {item.chefNotes}</p>
                      )}
                      {item.modifiers?.map((m) => (
                        <p key={m.modifierId} className="text-xs text-text-secondary">
            ...
            <div className="space-y-1.5">
              ...
              {parseFloat(String(order.discountAmount)) > 0 && (
                <div className="flex justify-between text-sm text-success">
                  <span>Discount</span>
                  <span>-{formatCurrency(parseFloat(String(order.discountAmount)))}</span>
                </div>
              )}
            ...
            <div className="space-y-2">
              {order.statusHistory?.map((h) => (
                <div key={h.id} className="flex items-center gap-3 text-sm">
                  ...
                  <span className="text-text-secondary">{formatTime(h.changedAt)}</span>
                  <OrderStatusBadge status={h.newStatus} />
                  {h.reason && <span className="text-text-disabled text-xs">· {h.reason}</span>}
                </div>
              ))}
            </div>
          </Card>
        ...
        <div className="space-y-4">
          <Card>
            <h2 className="text-base font-semibold text-text-primary mb-3">Order Info</h2>
            <div className="space-y-3">
              <div className="flex items-center gap-2 text-sm">
                <MapPin className="w-4 h-4 text-text-disabled" />
                <span className="text-text-secondary">
                  Type: <span className="font-medium text-text-primary">{order.type?.replace('_', ' ')}</span>
                </span>
              </div>
              {order.table && (
                <div className="flex items-center gap-2 text-sm">
                  <MapPin className="w-4 h-4 text-text-disabled" />
                  <span className="text-text-secondary">
                    Table: <span className="font-medium text-text-primary">{(order.table as any).name}</span>
                  </span>
                </div>
              )}
              {order.notes && (
                <div className="text-sm">
                  ...
                  <p className="text-text-primary bg-surface-secondary rounded p-2 text-xs">{order.notes}</p>
                </div>
              )}
            </div>
          </Card>
          ...
          {(transitions.length > 0 || canAddItems) && (
            <Card>
              <h2 className="text-base font-semibold text-text-primary mb-3">Actions</h2>
              <div className="space-y-2">
                {canAddItems && (
                  <Button
                    variant="secondary"
      ...
      {showAddItems && (
        <AddItemsModal orderId={orderId} onClose={() => setShowAddItems(false)} />
      )}
    </Page>
  );
}
