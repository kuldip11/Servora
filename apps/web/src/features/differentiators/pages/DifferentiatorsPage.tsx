...
export const DifferentiatorsPage = () => {
  ...
  const [availabilityFulfillment, setAvailabilityFulfillment] =
    useState("UNSCOPED");
  ...
  >("ALL");
  const [engineeringSort, setEngineeringSort] =
    useState<EngineeringSort>("volume");
  ...
  async function loadAvailability() {
    try {
      const response = await availabilityApi.dashboard<{
        ...
        ...(availabilityCause.trim()
          ? { cause: availabilityCause.trim() }
          : {}),
      });
    ...
    try {
      ...
      setMenuChoices(
        categories.flatMap((category) =>
          (category.menuItems ?? [])
            .filter(
              (item) => item.isPublished && item.status !== "DISCONTINUED",
            )
            .map((item) => ({
              id: item.id,
              name: item.name,
  ...
  async function explainOrder() {
    ...
    try {
      setExplain(
        await ordersApi.explain<Record<string, unknown>>(orderId.trim()),
      );
    } catch (error) {
      fail(error);
    } finally {
    ...
    <Page>
          ...
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-text-secondary">
                All availability comes from the authoritative resolver; this
                view only aggregates its output.
              </p>
            </div>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            <label className="text-sm font-medium text-text-primary">
              ...
              <select
                ...
                onChange={(event) => setAvailabilityChannel(event.target.value)}
              >
            ...
            <label className="text-sm font-medium text-text-primary">
              ...
              <select
                className={`mt-1 w-full ${DIFFERENTIATORS_SELECT_CLASS}`}
                value={availabilityFulfillment}
                onChange={(event) =>
                  setAvailabilityFulfillment(event.target.value)
                }
              >
            ...
            <label className="text-sm font-medium text-text-primary">
              ...
              <select
                className={`mt-1 w-full ${DIFFERENTIATORS_SELECT_CLASS}`}
                value={availabilityCause}
                onChange={(event) => setAvailabilityCause(event.target.value)}
              >
          ...
          <Card>
            <div className="grid gap-3 md:grid-cols-3">
              <label className="text-sm font-medium text-text-primary">
                ...
                <select
                  ...
                  onChange={(event) => {
                    ...
                    void loadEngineering(event.target.value);
                  }}
                >
              ...
              <label className="text-sm font-medium text-text-primary">
                ...
                <select
                  className={`mt-1 w-full ${DIFFERENTIATORS_SELECT_CLASS}`}
                  value={engineeringSort}
                  onChange={(event) =>
                    setEngineeringSort(event.target.value as EngineeringSort)
                  }
                >
      ...
      {tab === "explain" && (
        <Card>
          ...
          <p className="mt-1 text-sm text-text-secondary">
            Uses the order's immutable resolution timestamp and fire-time
            resolver snapshots.
          </p>
        ...
        <div className="space-y-4">
          ...
          {builderKind === "combo" ? (
            <Card>
              ...
              <p className="mt-1 text-sm text-text-secondary">
                Step 1: name the offer. Step 2: choose real menu items by name.
                Step 3: preview through the same server pricing stages used by
                order creation.
              </p>
                ...
                <div className="grid gap-3 md:grid-cols-3">
                  ...
                  <label className="text-sm font-medium text-text-primary">
                    ...
                    <input
                      ...
                      min="0"
                      max={
                        comboPolicy === "PERCENT_OFF_SUM" ? "100" : undefined
                      }
                      step="0.01"
                      value={comboValue}
                      onChange={(event) => {
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setComboSlots((current) => [
                        ...
                        {
                          id:
                            Math.max(0, ...current.map((slot) => slot.id)) + 1,
            ...
            <Card>
              ...
              <p className="mt-1 text-sm text-text-secondary">
                ...
                item/category/BOGO targeting remains in the full promotion
                editor.
              </p>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-4 md:items-end">
            <label className="text-sm font-medium text-text-primary">
              ...
              <select
                ...
                value={action}
                onChange={(event) =>
                  setAction(event.target.value as "VOID" | "COMP")
                }
              >
