...
export function MenuEngineeringPage() {
  ...
  const visibleRows = useMemo(() => {
    const filtered = quadrant === "ALL" ? [...rows] : rows.filter((row) => row.quadrant === quadrant);
    return filtered.sort((left, right) => {
      if (sort === "margin") return right.margin - left.margin;
      if (sort === "volume") return right.salesVolume - left.salesVolume;
      return left.menuItemName.localeCompare(right.menuItemName) || (left.variantName ?? "").localeCompare(right.variantName ?? "");
    });
  }, [quadrant, rows, sort]);
  ...
  return (
    <Page>
      ...
      {error ? (
        <Card className="border-danger/30 bg-danger-surface"><p className="text-sm font-semibold text-danger">Menu engineering unavailable</p><p className="mt-1 text-sm text-text-secondary">{error}</p></Card>
      ) : loading && rows.length === 0 ? (
        <div className="flex min-h-40 items-center justify-center"><Spinner className="h-6 w-6" /></div>
      ) : visibleRows.length === 0 ? (
        <Card><p className="text-sm text-text-secondary">No menu items match the selected analysis scope.</p></Card>
      ) : (
        <div className="grid gap-4 lg:grid-cols-2">
          {visibleRows.map((row) => (
            <Card key={`${row.menuItemId}:${row.variantName ?? ""}`}>
              <div className="flex items-start justify-between gap-3">
                <div><h2 className="font-semibold text-text-primary">{row.menuItemName}{row.variantName ? ` — ${row.variantName}` : ""}</h2><p className="mt-1 text-sm text-text-secondary">{row.salesVolume} sold · {formatCurrency(row.margin)} contribution margin{typeof row.marginPercent === "number" ? ` · ${row.marginPercent.toFixed(1)}%` : ""}</p></div>
                <Badge variant={row.quadrant === "STAR" ? "success" : row.quadrant === "DOG" ? "danger" : "warning"}>{row.quadrant}</Badge>
              </div>
              <div className="mt-4 rounded-lg bg-surface-secondary p-3"><p className="text-xs font-semibold uppercase tracking-wide text-text-disabled">Suggested action</p><p className="mt-1 text-sm text-text-secondary">{row.recommendation}</p></div>
            </Card>
          ))}
        </div>
      )}
